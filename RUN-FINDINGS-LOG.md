...
| 12 | ~~**`slide_generator returned unknown segment_id(s)`**~~ **CLOSED 2026-08-25 (D133).** 4 of 20 PDFs failed this way in the completed run (`dense_text_with_headers`, `table_heavy_small`, `image_heavy_small`, `image_heavy_grid`). Root cause: the LLM copies the real segment_id PERFECTLY every time, then appends `": <a better title it inferred from the summary>"` — because the prompt's own `segment_id={id}: {title} — {summary}` line format teaches it that a segment_id value looks like `"token: description"`. Only fires when the real id+title is generic/bare (`"Document"`, `"1.1 Subsection"`) — every other fixture's real headings are specific enough that the model has nothing to "improve." `lesson_planner_node` has the identical risky prompt shape but is protected by a retry loop already added for D77 (a related problem); `slide_generator_node` had no equivalent retry. **Fixed** by applying that same proven D77 retry pattern (`_SLIDE_GENERATOR_MAX_ATTEMPTS=3`). **Live-reverified 2026-08-25: all 4 fixtures now pass (0/4 failures)**, and the retry mechanism was observed actually firing on real production data (`image_heavy_small`'s log + a matching real Langfuse trace showing 2 back-to-back slide-deck completion calls). Full detail: `D133-FIX-TRACKER.md` / `D133` in `DEFECT-REGISTER.md`. | Live 20-PDF eval run, 2026-08-24 (final: 16/20 passed, 4/20 this same failure, $10.64 total, 2h40m); re-verification run 2026-08-25 (4/4 passed, $4.13 total) | High (4/20 real failures, mechanism confirmed to be generic LLM behavior, not fixture-specific) — now resolved | **Closed.** Guarded by 2 new unit tests, RED-GREEN verified, plus real live re-verification. |
