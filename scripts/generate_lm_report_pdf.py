"""Generate a PDF for the Learner Mode consolidated manager report."""
...
MD_PATH = REPO / "docs" / "learner-mode-manager-report-2026-07-30.md"
PDF_PATH = REPO / "docs" / "learner-mode-manager-report-2026-07-30.pdf"
...
def md_inline(text: str) -> str:
    ...
    # escape XML special chars first (except those we'll inject)
...
def on_page(canvas, doc):
    ...
    canvas.drawString(1.8 * cm, h - 0.72 * cm,
                      "TransformED AI — Learner Mode: Consolidated Manager & Founder Report")
    canvas.setFont("Helvetica", 8)
    canvas.drawRightString(w - 1.8 * cm, h - 0.72 * cm, "2026-07-30")
    # bottom bar
    canvas.setFillColor(RULE_COLOR)
    canvas.rect(0, 0, w, 0.85 * cm, fill=True, stroke=False)
    canvas.setFillColor(TEXT_MUTED)
    canvas.setFont("Helvetica", 7.5)
    canvas.drawString(1.8 * cm, 0.3 * cm, "CONFIDENTIAL — For Engineering Manager & Founder only")
...
def build_pdf(md_path: Path, pdf_path: Path):
    ...
    doc = SimpleDocTemplate(
        ...
        topMargin=1.8 * cm,
        bottomMargin=1.4 * cm,
        title="Learner Mode — Consolidated Manager & Founder Report",
    ...
    def peek(offset=1):
        return lines[i + offset] if i + offset < len(lines) else ""
    ...
    while i < len(lines):
        ...
        # ── Meta block (lines starting with ** up to first ---) ──
        if line.startswith("**Feature:**") or line.startswith("**Feature window:**") \
                or line.startswith("**Report date:**") or line.startswith("**Prepared by:**") \
                or line.startswith("**Audience:**") or line.startswith("**Sources:**"):
        ...
        if line.startswith("|"):
            ...
            if rows:
                n_cols = len(rows[0])
                pw = A4[0] - 3.6 * cm
                # Heuristic column width distribution
                if n_cols == 2:
                    cw = [pw * 0.55, pw * 0.45]
                elif n_cols == 3:
                    cw = [pw * 0.22, pw * 0.48, pw * 0.30]
                elif n_cols == 4:
                    cw = [pw * 0.18, pw * 0.42, pw * 0.18, pw * 0.22]
                elif n_cols == 5:
                    cw = [pw * 0.14, pw * 0.36, pw * 0.12, pw * 0.14, pw * 0.24]
                elif n_cols == 6:
                    cw = [pw / 6] * 6
                else:
                    cw = [pw / n_cols] * n_cols
        ...
        # Verdict / integration update / warning paragraphs (bold-led lines)
        stripped = line.strip()
        ...
        if stripped.startswith("**Integration update"):
            story.append(Paragraph(md_inline(stripped), S["update_box"]))
            i += 1
            continue

        if stripped.startswith("**This section is included"):
