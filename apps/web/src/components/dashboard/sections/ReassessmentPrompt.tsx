...
const DISMISS_KEY_PREFIX = "dismissed_reassessment_prompt_";

function dismissKey(userId: string, sessionCount: number): string {
    return `${DISMISS_KEY_PREFIX}${userId}_at_session_${sessionCount}`;
}

function isDismissed(userId: string, sessionCount: number): boolean {
    if (typeof window === "undefined") return false;
    try {
        return window.localStorage.getItem(dismissKey(userId, sessionCount)) !== null;
    } catch {
        return false;
    }
}

function markDismissed(userId: string, sessionCount: number) {
    if (typeof window === "undefined") return;
    try {
        window.localStorage.setItem(dismissKey(userId, sessionCount), "1");
    } catch {
...
export function ReassessmentPrompt() {
    ...
    const shouldShow = dna !== null && !dismissed && !isDismissed(dna.user_id, dna.session_count);

    function handleDismiss() {
        if (dna !== null) markDismissed(dna.user_id, dna.session_count);
        setDismissed(true);
    }
