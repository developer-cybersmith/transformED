...
def _build_supabase_tb(
    ...
    count_mock.select.return_value.eq.return_value.eq.return_value.execute.return_value.count = (
        attempt_count
    )
...
async def test_happy_path_returns_teachback_result(mock_to_thread, mock_score_teachback) -> None:
    ...
    grade_teachback returns TeachbackResult with correct values
    ...
    assert result.ces_contribution == pytest.approx(
        round((_MOCK_TB_RESULT.score / 100.0) * 0.25 * 100, 4)
    )
...
async def test_rubric_scores_contains_three_keys(mock_to_thread, mock_score_teachback) -> None:
    """rubric_scores must contain 'accuracy', 'completeness', 'clarity' as
    ...
    for key, val in result.rubric_scores.items():
        assert isinstance(val, str), (
            f"rubric_scores['{key}'] must be str label, got {type(val).__name__}: {val!r}"
        )
...
@pytest.mark.unit
async def test_feedback_low_score_praise_and_correction(
    mock_to_thread, mock_score_teachback
) -> None:
...
async def test_score_teachback_called_with_correct_args(mock_to_thread) -> None:
    ...
    assert captured_kwargs["key_concepts"] == ["chlorophyll", "ATP"], (
        f"key_concepts must be [j['term'] for j in jargon], "
        f"got {captured_kwargs.get('key_concepts')!r}"
    )
    assert captured_kwargs["response_text"] == "Chlorophyll captures sunlight for photosynthesis."


@pytest.mark.unit
async def test_llm_provider_constructed_with_lesson_id(
    mock_to_thread, mock_score_teachback
) -> None:
    """AC 7: OpenAILLMProvider is constructed with lesson_id so cost is tracked per lesson."""
    from unittest.mock import patch as _patch

    provider_mock_cls = MagicMock()
    provider_mock_cls.return_value = MagicMock()
...
async def test_response_text_written_to_db(mock_to_thread, mock_score_teachback) -> None:
    ...
    lesson_mock = MagicMock()
    lesson_ex = lesson_mock.select.return_value.eq.return_value.maybe_single.return_value.execute
    lesson_ex.return_value.data = {"content": _LESSON_CONTENT}
    count_mock = MagicMock()
...
@pytest.mark.unit
async def test_teachback_duplicate_attempt_returns_409(
    mock_to_thread, mock_score_teachback
) -> None:
...
def test_unauthenticated_request_returns_403() -> None:
    """No Authorization header yields 401/403.

    HTTPBearer(auto_error=True) fires before business logic.
    """
...
def test_response_text_too_long_rejected() -> None:
    ...
    assert resp.status_code == 422, (
        f"Expected 422 for 4001-char response_text, got {resp.status_code}"
    )
...
def test_response_text_at_max_length_accepted(monkeypatch) -> None:
    ...
    assert resp.status_code == 200, (
        f"Expected 200 for 4000-char response_text, got {resp.status_code}"
    )
...
def test_response_text_single_char_accepted(monkeypatch) -> None:
    ...
    with patch("app.core.db.get_supabase", return_value=MagicMock()):
        resp = _client.post("/api/assessment/teachback", json=payload)
    assert resp.status_code == 200, (
        f"Expected 200 for single-char response_text, got {resp.status_code}"
    )
...
async def test_session_wrong_user_returns_404(mock_to_thread, mock_score_teachback) -> None:
    ...
    supabase = _build_supabase_tb(
        session_data=other_session, lesson_data={"content": _LESSON_CONTENT}
    )
    with pytest.raises(HTTPException) as exc_info:
        await grade_teachback(
            session_id="sess-001",
...
@pytest.mark.unit
async def test_feedback_boundary_score_89_praise_and_correction(
...
async def test_feedback_boundary_score_90_praise_only(mock_to_thread, monkeypatch) -> None:
    """AC 7: score=90 with non-empty correction.

    The model_validator clears correction, so feedback = praise only.
    """
...
@pytest.mark.unit
async def test_attempt_number_count_none_defaults_to_1(
    mock_to_thread, mock_score_teachback
) -> None:
...
def test_xml_injection_does_not_escape_delimiter_region() -> None:
    ...
    assert p.count("<student_response>") == 1, "Injection must not add extra opening tags"
...
async def test_rubric_scores_are_descriptive_labels(mock_to_thread, monkeypatch) -> None:
    ...
    assert result.rubric_scores["completeness"] == "Developing", (
        f"completeness_score=70 must map to 'Developing', "
        f"got {result.rubric_scores['completeness']!r}"
    )
