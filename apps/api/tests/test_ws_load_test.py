"""Unit tests for the pure aggregation logic of scripts/ws_load_test.py (Dev 4 — Sprint 4 ws_load_test).
...
live (see docs/sprint4-ws-load-test.md); it is intentionally NOT exercised in the unit suite to avoid
binding real sockets in CI.
...
def test_summarize_counts_drops() -> None:
    """drops = sessions_requested - connected (covers never-connected AND errored-before-connect)."""
    results = [
...
def test_summarize_latency_percentiles() -> None:
    ...
    results = [
        SessionResult(
            "a", connected=True, signals_sent=5, acks=5,
...
def test_summarize_counts_missed_acks() -> None:
    results = [
        ...
        SessionResult("b", connected=True, signals_sent=3, acks=3, missed_acks=0, latencies=[0.01, 0.01, 0.01]),
    ]
...
def test_summarize_mid_run_death_counts_as_drop() -> None:
    ...
    results = [
        SessionResult("a", connected=True, signals_sent=60, acks=60, latencies=[0.01] * 60),  # healthy
        SessionResult("b", connected=True, signals_sent=30, acks=30, latencies=[0.01] * 30,
                      error="ConnectionClosedError()"),  # connected, then killed mid-run
    ]
    s = summarize(results, sessions_requested=2)
...
def test_summarize_passed_flag() -> None:
    ...
    missed = [SessionResult("a", connected=True, signals_sent=2, acks=1, missed_acks=1, latencies=[0.01])]
