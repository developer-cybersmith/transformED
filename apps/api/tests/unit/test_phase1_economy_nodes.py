...
def _no_checkpoint_infra():
    ...
    _jobs_single = mock_jobs_table.select.return_value.eq.return_value.single
...
class TestAC3QuizGenerator:
    ...
    async def test_out_of_range_correct_index_is_rejected(self) -> None:
        ...
        mock_batch = self._batch(self._q(question="Which is a memory technique?", correct_index=7))
        mock_provider = AsyncMock()
        mock_provider.complete_structured.return_value = mock_batch
...
class TestAC6NarrationGenerator:
    ...
    async def test_pacing_guard_rejects_script_too_dense_for_target_duration(self) -> None:
        ...
        assert result["narration_scripts"] == [], (
            "a script implying 20 words/sec against a 10s target duration "
            "must be rejected (cap 15/sec)"
        )
...
class TestAC7CostCeilingEndToEnd:
    ...
    async def test_ceiling_breach_produces_failed_status_with_cost_ceiling_exceeded_prefix(
        ...
        _lessons_single = mock_lessons_table.select.return_value.eq.return_value.single
        _lessons_single.return_value.execute.return_value.data = {
            "user_id": FAKE_USER_ID,
            ...
            patch(
                ...
                new=AsyncMock(
                    side_effect=RuntimeError(
                        f"cost ceiling exceeded before Phase 1 economy-node dispatch "
