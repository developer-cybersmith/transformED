"""Guard (D144): HeyGen must not reappear.
...
`docs/DEFECT-REGISTER.md` D144). This guard is the actual RED/GREEN pivot for
...
def test_heygen_avatar_provider_file_does_not_exist() -> None:
    assert not (_APP_DIR / "providers" / "avatar" / "heygen.py").exists(), (
        ...
        "dead/unwired code (D144), not merely deprecated"
    )
...
def test_heygen_avatar_provider_class_does_not_appear_anywhere() -> None:
    ...
    assert not hits, f"HeyGenAvatarProvider reintroduced in: {hits} — see D144"
...
def test_avatar_provider_abc_does_not_appear_anywhere() -> None:
    ...
    assert not hits, f"AvatarProvider ABC reintroduced in: {hits} — see D144"
...
def test_heygen_api_key_setting_does_not_exist() -> None:
    ...
    assert not hits, f"heygen_api_key config field reintroduced in: {hits} — see D144"


@pytest.mark.unit
def test_avatar_clips_bucket_is_no_longer_required() -> None:
    """The bucket itself is NOT retired from the frozen migration (D144) —
    ...
    assert "avatar-clips" not in REQUIRED_BUCKETS, (
        ...
        "references it since HeyGen was removed (D144)"
    )
