...
async def _extract_text_only(pdf_bytes: bytes) -> dict[str, Any]:
    ...
    The truncation numbers come from the detector, not from the extractor —
    full text for the pages the contents scan can reach, `TITLE_WINDOW` characters
    everywhere else, plus `TITLE_TAIL_WINDOW` characters from each truncated
    page's tail (D115/D116 — some publishers emit a chapter-opener title after
    the body text, so it lands past `TITLE_WINDOW` and was otherwise discarded
    before the detector ever saw it). Passing them keeps the extractor ignorant
    of detection policy while stopping a 1,151-page book from shipping an
    unbounded amount of JSON through a pipe for the sake of 800-character
    lookups.
    """
    from app.modules.content.chapter_detection.gate import CONTENTS_SCAN_PAGES
    from app.modules.content.chapter_detection.text import TITLE_TAIL_WINDOW, TITLE_WINDOW
