...
# Story 3-28 (Learner Mode sprint Task 1): tier -> quiz question count band per
# segment. T1 = full-depth comprehension (3-5 Qs), T2 = standard (2-3 Qs),
# T3 = refresher (1-2 Qs). Deliberately a module-level constant (not in
# config.py) — matches _TIER_TOTAL_SLIDE_BAND's pattern; quiz counts have no
# constraint requiring runtime tunability (unlike CES weights which must sum to 1).
...
async def quiz_generator_node(state: PipelineState) -> PipelineState:
    ...
    raw_questions = list(response.questions)[:n_max]  # AC defensive: truncate super-large batches
