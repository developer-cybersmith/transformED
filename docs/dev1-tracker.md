...
**Last updated:** 2026-09-25 (added #233 piece 1 — topic-selection step, implementation complete, review pending — under "Post-Launch / Platform Architecture"; 2026-09-22: added #236 under the same section — not a sprint/task id, see that section; #232/S4-9 added below)
...
| Sprint 4 | Weeks 8–9 (Jul 31 – Aug 13) | 9 | 2 | 1 | 6 |
...
- [ ] **S4-6 Fly.io backups + disaster recovery tested** — renamed from "Railway backups" 2026-09-05 (D165); Railway was retired 2026-08-14, platform is Fly.io + Upstash Redis. DR work itself is still not started.
...
- [ ] **S4-9 60db.ai TTS tier — new primary, ahead of Sarvam (Story 232, GitHub issue #232)** ⚠️ PARTIAL — 2026-09-22
  - Branch `feature/232-sixtydb-tts-tier`, story file `docs/stories/232-sixtydb-tts-tier.md`. Chain: **60db → Sarvam → Azure → Browser** (never hard-fails) — additional tier, not a cutover, per the issue's locked decision.
  - **Scope correction found live, not assumed:** issue #232 claimed "60db returns word-level timestamps natively." A real call to `POST https://api.60db.ai/tts-synthesize` (real key/voice_id) returned only `audioContent` + `conditioning` — zero timing fields. Timestamp/caption work dropped from scope entirely; this story is audio-only. Same live call also caught the skill's own reference docs describing the wrong response shape (`{"result": {...}}` wrapper that doesn't exist) — parser handles both defensively.
  - Implementation complete: `SixtyDbTTSProvider` (`apps/api/app/providers/tts/sixtydb.py`), config settings, `AudioProvider` enum +`"sixtydb"` (frozen contract — **4-developer review not yet obtained**), `.env.example` fix. Full regression **1521 passed, 0 failures**; ruff clean; `mypy app` (repo-wide) 4 pre-existing/unrelated errors, 0 new.
  - **All 6 required review layers now complete** across 3 rounds: `/code-review` (high effort, 5 findings/4 fixed), `/bmad-code-review`'s 4 built-in layers (Blind Hunter, Edge Case Hunter, Acceptance Auditor, Scale & Load Hunter), and the 4 CLAUDE.md-named layers the skill doesn't supply (Story Quality, Test Coverage, AC Completeness, Process Integrity). Fixed a HIGH-severity cost-integrity bug (retrying a whole multi-chunk request as one unit could re-bill already-succeeded chunks while cost was counted once, silently under-billing against the $3.00 ceiling). Registered **D168** (unconfirmed 60db per-char cost), **D169** (identical pre-existing bug found in `sarvam.py` itself, out of this story's scope to fix), and **D170** (unrelated, pre-existing `.env.example`/`config.py` CES-weight drift on `main`, found as a review byproduct).
  - **Blocking before merge (cannot be satisfied by this story alone):** a human read of the review findings, and the 4-developer sign-off required for the frozen `AudioProvider` contract change (CLAUDE.md/PRD §16).
...
## Post-Launch / Platform Architecture (GitHub issues, not sprint-tracker tasks)

> Work tracked by GitHub issue number rather than a sprint/task id — the content pipeline
> continues to evolve after Week 10 launch. See `docs/stories/{issue-number}-{slug}.md` for
> each item's full story file.

- [x] **#236 Narration continuity — move narration_generator after lesson_planner, add narration_stitch_node** — ✓ 2026-09-21 (Story 236, branch `feature/236-narration-post-planner-ordering`)
  - `apps/api/app/modules/content/pipeline/graph.py` *(narration_generator moved from the Phase-1 economy fan-out into a new post-planner fan-out dispatched from `slide_generator`, driven by `lesson_plan.segments`; `lesson_plan.segments[i].continuity_notes` added; new `narration_stitch_node` — transition/duplicate-phrasing polish pass + the Story 3-37 narration char cap moved here from `tts_node`; `tts_node`/`package_builder_node` repointed to the new `narration_scripts_final` field)*
  - `docs/DEFECT-REGISTER.md` *(new **D167** — `continuity_notes` is authored per-batch, not lesson-wide, once a chapter exceeds `lesson_planner_batch_size`; registered, not fixed, out of this story's scope)*
  - Blocks/unblocks: was the verified architectural prerequisite for issue #233 (mandatory 15/30/45-min fixed slide structures + continuous transcript) — narration can now see the finished lesson outline and callback across sections, which #233's own text names as a hard blocker on this exact change.
  - Full 6-agent `/bmad-code-review` (5 parallel agents, all 6 CLAUDE.md layers) completed 2026-09-21 — 2 confirmed findings fixed (an unguarded exception in `narration_stitch_node`'s LLM call that could crash an otherwise-complete lesson; a stale comment about `continuity_notes`' per-batch limitation, corrected and registered as D167), 5 new tests added for previously-uncovered guard paths.
  - **AC:** All 19 ACs verified by actual test execution (not asserted from memory) — see `docs/stories/236-narration-post-planner-ordering.md`. Full gating-scope regression (`tests/unit` + `tests/integration -m "not postgres"`): **1527 passed, 6 skipped**, zero failures ✅. `ruff check`/`ruff format --check`/repo-wide `mypy app`: clean, zero new issues vs. `main` ✅.

- [ ] **#233 (piece 1 of 4) Topic-selection step — collapse chapter sections into 1 topic (T3) or 2 topics (T1/T2)** — implementation complete 2026-09-25, review pending (Story 233, branch `feature/233-topic-selection-step`)
  - `apps/api/app/schemas/lesson.py` *(new `TIER_TOPIC_COUNT`: T1/T2 → 2, T3 → 1)*
  - `apps/api/app/modules/content/pipeline/nodes/structure_detection.py` *(new `merge_section_range` — text-preserving N-section merge built on the existing `_merge_two` primitive)*
  - `apps/api/app/config.py` *(`section_body_max_chars` re-derived 6,000 → 45,000 — the prior cap was sized for one of up to 15 small sections; topic-selection can now make "one section" up to ~15× bigger)*
  - `apps/api/app/modules/content/pipeline/graph.py` *(new `topic_selection_node`, `_StructureTopicSplitLLM`, `_topic_selection_llm_split`, `_topic_selection_midpoint_split`; wired `embed → topic_selection → <Phase-1 fan-out>`, was `embed → <fan-out>` directly; module docstring's node diagram updated to 17 nodes)*
  - Fixed 3 tests whose premises predated topic-collapse (a hardcoded quiz-weight bound tied to the old cap; a fan-out test that needed `topic_selection_node` stubbed; and — the notable one — `test_tier_changes_the_delivered_package`'s "segment count must be identical across tiers" invariant, which this story exists to break, replaced with the real new contract T3=1/T1=T2=2 verified end-to-end through the real graph). See the story's Dev Agent Record for full reasoning.
  - Piece 1 of 4 (issue #233's own recommended split): topic-selection step (this), split-screen slide schema, continuous transcript, per-duration slide tables — the other 3 pieces are not started.
  - Related but explicitly out of scope, split into a separate follow-up issue after a full line-by-line wiring audit: **#249** — `book_context`/`chapter_context` (the two onboarding-style forms that already exist) reach only 1-3 of 8 content-generation nodes today; not touched by this story.
  - **AC:** All 14 ACs implemented; guard-test survey, RED-phase tests, and full regression run and documented in `docs/stories/233-topic-selection-step.md`'s Dev Agent Record. Full gating-scope regression (`tests/unit` + `tests/integration -m "not postgres"`): **1663 unit + 20 integration passed**, 6 skipped, 1 pre-existing unrelated failure (confirmed via `git stash` against this branch's base commit) — zero regressions from this change. `ruff check`/`ruff format --check`: clean repo-wide. `mypy app`: clean on every file this story touched (4 pre-existing errors elsewhere, unrelated). 6-layer adversarial review and PR not yet done.
