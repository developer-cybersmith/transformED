...
- [x] **S3-1 Eval harness expanded to 20 PDFs** — ✓ 2026-08-14
  ...
  - **AMENDED 2026-08-20 — Sarvam credits were NOT the only blocker.** A real live attempt
    (2026-08-18/19) surfaced that this harness has been unable to run AT ALL since Story 1-13
    (chapter-scoped generation) landed 2026-08-04 — `run_eval()` predates that story and never
    threaded a real `chapter_id`, which `extract_node` now hard-requires. This is a second,
    independent, structural blocker the "blocked on Sarvam credits" framing above never
    surfaced (fixing credits alone would still have failed instantly). **Fixed as D124**
    (`docs/DEFECT-REGISTER.md`, PR #150, merged 2026-08-19) — the harness now runs real
    `book_ingest_job` chapter detection before building the lesson. Two more real gaps found
    and fixed in the same pass: `init_redis()` was never called outside the FastAPI lifespan
    (every PDF failed in <1s with "Redis pool is not initialised"), and the placeholder
    all-zeros `user_id` violated `books_user_id_fkey` (fixed with a real dedicated
    `eval-harness@internal.transformed.local` auth user). **Sarvam credits were separately
    topped up/rotated by the user this session.** Both blockers are now cleared — **the actual
    20-PDF `--run-live-eval` run is the sole remaining step**, not yet executed (real spend,
    needs explicit authorization before running).
...
- [ ] **S3-2 Prompt iteration from eval results**
  ...
  - **AMENDED 2026-08-20 — first real pass taken, still not this AC's bar.** With S3-1's live
    run still not executed, this AC's literal requirement (real before/after Langfuse scores)
    remains unmet — **not marking this done.** What was done instead: `slide_generator_node`'s
    prompt was tuned against a real, already-known gap in what the eval harness's OWN scorer
    checks (`tests/evals/scoring.py`'s `score_slide_quality` fails any bullet over 200 chars as
    "wall of text"; the prompt gave zero guidance on bullet length, so a paragraph-length bullet
    passed every existing guard and then failed the eval score on every run) — evidence-based
    tuning against the scorer's documented dimensions, explicitly NOT the "real before/after
    Langfuse scores" this AC requires. Shipped as D125 (`docs/DEFECT-REGISTER.md`, PR #151,
    merged 2026-08-19) with a code-level truncation guard as a backstop (a prompt is a request,
    not a guarantee) — PR review (Dev 2) caught a real off-by-one in that guard before merge
    (truncated length landed at 201 chars, still failing the scorer's own `> 200` check; fixed
    and amended in the same register entry). **With D124 fixed and Sarvam credits topped up
    (see S3-1's amendment), this AC's actual blocker — a live eval run — is now clear to
    attempt; D125's slide-prompt change is a candidate first "before" data point once it runs.**
