...
**Last updated:** 2026-08-26 — Story 5-5 (RLS audit) done, live-verified against the real project
...
| 5-3 | ~~Stripe Checkout integration~~ **SUPERSEDED 2026-08-31** — real payment provider is Razorpay (PR #157, `razorpay-backend-endpoints-dev3`, Epic 4/Story 4-1), not Stripe. This Stripe implementation was built without visibility into that parallel effort; PR #158 closed, branch deleted, `main` never touched. See `docs/dev1-tracker.md` S4-3 for the full correction. | — | 5-5 (partial, via PR #157 instead), 5-7 |
...
| 5-5 | RLS security audit on all Supabase tables | `done` — all 15 live tables live-verified for SELECT (real minted sessions via Admin API, since project uses asymmetric JWT signing keys); full write-phase live-tested on 2 tables + 2 join-table INSERT-rejection spot-checks (11 tables' writes rest on migration text, D143); consent-gate live for 2/4 states (D143); storage + function-grant audits all live incl. direct object GET; 8-layer code review complete, D141 (open) + D142 (accepted) + D143 (accepted residual scope) filed; `docs/security/rls-audit.md` committed; not yet pushed/merged | 5-3 (partial — `lesson_access`/`stripe_events` not yet on this branch) | Epic-5 DoD → W10-1 |
...
**Totals:** 8 stories · 2 done (5-4, 5-5) · 1 superseded (5-3 — real work is PR #157/Razorpay, outside this sprint's story numbering) · 0 in-progress · 0 review · 5 ready-for-dev
...
**5-5 — RLS audit. DONE, live-verified against the real project, not the shim.** Self-minted HS256 JWTs turned out to be cryptographically impossible against this project — its JWKS endpoint confirmed a migration to asymmetric ES256 signing keys, so real sessions for owner/stranger principals were minted instead via the service-role Admin API's `generate_link` + redirect-token exchange. All 15 tables checked live across owner/stranger/anon (SELECT), plus a full self-cleaning write-phase (insert/reject/accept/delete) on two empty tables (`user_notification_preferences`, `attention_events`) — **no RLS gap found in anything actually exercised**; every "0 rows" stranger result was cross-checked against service-role ground truth to rule out false negatives. Storage buckets and both privileged RPC functions (`merge_lesson_job_node_output`, `increment_learner_dna_session_count`) live-tested directly too — both correctly reject anon and real authenticated users, including a direct object GET (not just bucket/object listing). **8-layer `/bmad-code-review` (2026-08-26) — 6 of 8 layers independently converged on the same finding:** an earlier draft overstated live-testing scope ("full CRUD on all 15 tables", "4 consent states tested"). Corrected with real additional evidence rather than just softer wording: a live consent-gate reject test (unconsented account, real session, real 403), 2 live cross-account INSERT-rejection spot-checks on join-based tables (`quiz_attempts`, `chapters`), and the direct storage GET test above. A factual error (`books`' ownership column mis-cited as `uploaded_by`, real column is `user_id`) was also caught and fixed. Three findings registered: **D142** (accepted, not a defect) — `attention_events` DELETE/UPDATE don't gate on `attention_consent`, judged intentional since an erasure right shouldn't depend on active consent. **D141** (open) — `user_consents.consent_type` CHECK constraint doesn't include `'data_processing'`, which Epic-5's DoD requires at signup; confirmed by grep that zero code writes it — entirely unimplemented, needs a future migration + signup-flow wiring, out of this audit's scope to build. **D143** (accepted residual scope) — 11 of 15 tables' non-SELECT commands and 2 of 4 consent-gate states were not live-tested, registered explicitly rather than left as a silent overclaim. Full report: `docs/security/rls-audit.md`.

**Renumbering note (added while resolving PR #160's merge into `main`, 2026-09-02):** this branch's D138/D139/D140 were assigned before `main` independently registered its own D138 (`upload_lesson` blocking I/O) and D139 (`generate_chapter_lesson` blocking I/O) via PRs #170/#171 — both landed first. Rather than perpetrate a fourth ID collision on top of the ones this same register already documents (D67, D74/D75/D76, D87), this story's three entries were renumbered to **D141/D142/D143** (consent_type gap / DELETE-UPDATE asymmetry / audit scope) at merge time, before either number had shipped in any commit message, test file, or cross-reference outside this branch's own docs. All mentions across this file, `docs/DEFECT-REGISTER.md`, `docs/dev1-tracker.md`, `docs/security/rls-audit.md`, and `docs/stories/5-5-rls-security-audit.md` were updated together.
...
- [x] RLS audit report committed; every table's policy verified; DPDP `consent_type` gap resolved or explicitly registered — **5-5** (`docs/security/rls-audit.md`; consent_type gap registered as D141 (open, not resolved); write-path/consent-state testing scope registered as D143 rather than overclaimed)
...
| 2026-08-26 | **Story 5-5 (S4-5, RLS security audit) implemented and done.** Branch `sprint4/s4-5-rls-audit` cut from `main` (story-only commit first, per §0). `docs/sprint4-plan.md` brought forward from `sprint4/s4-4-rate-limit-per-route` (its most recent copy across branches) before this update; 5-3's row corrected to `done` here based on its own branch's tracker state, since the two branches never saw each other's commits. **Real pivot found live, not assumed:** Task 1.2's planned self-minted-HS256-JWT approach is cryptographically impossible against this project — its JWKS endpoint confirmed a migration to asymmetric ES256 signing keys. Real owner/stranger sessions were minted instead via the service-role Admin API's `generate_link` + redirect-token exchange (no password, no email sent). All 15 tables verified live across 3 principals (SELECT), plus a full self-cleaning write-phase (accept + reject paths) on two empty tables — zero RLS gaps found; every stranger "0 rows" result cross-checked against service-role ground truth first. Storage buckets and both privileged RPC functions live-tested directly too. Two findings filed: D142 (accepted — `attention_events` DELETE/UPDATE consent-check asymmetry is intentional, an erasure right shouldn't require active consent) and D141 (open — `user_consents.consent_type` missing Epic-5's DoD-required `'data_processing'` value, confirmed entirely unimplemented by grep). `docs/security/rls-audit.md` written and committed. Not yet pushed to remote or merged. |
| 2026-08-26 | **Story 5-5 8-layer `/bmad-code-review` complete.** 6 of 8 independent layers (Acceptance Auditor, Evidence Rigor, AC Completeness, Story Quality, Edge Case Hunter, Scale & Load Hunter) converged on the same finding: the first draft's Completion Notes and the audit report itself overstated live-testing scope — "full CRUD verified for all 15 tables" and "4 consent states tested" described more than was actually run (SELECT was broad; full write-lifecycle testing was 2 tables; consent-gate testing was 1 state). Rather than just softening the language, closed the gap with real additional live evidence: a consent-gate reject test using a different real account (`attention_consent=false`, no `user_consents` row, real `403`, zero residue), 2 live cross-account INSERT-rejection spot-checks on join-based tables (`quiz_attempts`, `chapters`, both real `403`, zero residue confirmed via service-role re-check), and a live direct-GET (not just LIST) storage test (anon + real non-owner both `404 NoSuchKey`). Evidence Rigor also caught a real factual error — `books`' ownership predicate was cited as `uploaded_by = auth.uid()`; the real column, confirmed directly against the migration, is `user_id` — fixed. Process Integrity caught an inaccurate citation (`CLAUDE.md` doesn't literally name `'attention_capture'`, only the epic doc does) — fixed in both the audit report and D141. Blind Hunter's framing of the live-JWKS/Admin-API testing methodology as an "auth-bypass" concern was reviewed and judged mischaracterized (it's the intended, already-approved live-verification methodology, not a newly introduced risk; the app's dual HS256/ES256 verify path is pre-existing, correct-by-design, and out of this story's scope to change) — not actioned. Residual scope (11/15 tables' non-SELECT commands; 2/4 consent states) registered as **D143**, not silently left as an overclaim. `docs/DEFECT-REGISTER.md`, `docs/security/rls-audit.md`, `docs/stories/5-5-rls-security-audit.md`, and this file all updated to match the corrected scope. |
| 2026-08-31 | **Story 5-3 (Stripe) superseded — corrected, not just updated.** PR #157 ("Story 4-1 — Razorpay Payment Backend", branch `razorpay-backend-endpoints-dev3`, Tanmay Gupta, opened 2026-08-27 08:10 — before this Story 5-3 work existed) is the team's real, live payment-provider decision: Razorpay, not Stripe, fitting this product's India-first deployment (UPI, INR pricing). PR #157's own description confirms the pivot: "S4-02 in dev2 tracker still describes Stripe Checkout Redirect. Razorpay is fundamentally different." Story 5-3's Stripe implementation (PR #158) was built without visibility into this parallel effort — same module path (`apps/api/app/modules/payments/`), same core table (`lesson_access`), wrong provider. PR #158 closed 2026-08-27 with an explanatory comment, branch `sprint4/s4-3-stripe-checkout` deleted both locally and on `origin` — confirmed first that the PR was never merged and `main` has zero trace of the Stripe payments code. `docs/dev1-tracker.md`'s S4-3 line and this file's dashboard/totals corrected on the one branch (`sprint4/s4-5-rls-audit`) that had carried the stale "done" claim forward; `sprint4/s4-4-rate-limit-per-route` and `sprint4/s4-8-imagen-fallback` never had it. `docs/stories/5-3-stripe-checkout-integration.md` and its D136/D137 register entries are kept as historical record (the 8-layer review's money-losing-webhook-bug finding is a real, reusable lesson for reviewing Razorpay's own webhook idempotency) but must not be read as current status. |
| 2026-09-02 | **Merge conflict with `main` resolved (PR #160 → main).** `main` had independently landed its own D138 (`upload_lesson` blocking I/O, PR #170) and D139 (`generate_chapter_lesson` blocking I/O, PR #171) between when this branch was cut and when it merged — a genuine ID collision with this story's own D138/D139/D140. Renumbered this story's three entries to **D141/D142/D143** (consent_type gap / DELETE-UPDATE asymmetry / audit scope) across every file that named them, rather than add a fourth undisambiguated collision to the register's existing D67/D74-76/D87 ones. `docs/dev1-tracker.md`'s Quick Status Dashboard reconciled to reflect both S4-4 (#159) and S4-5 (#160) done. |
