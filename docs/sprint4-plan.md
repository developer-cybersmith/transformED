...
| 5-5 | RLS security audit on all Supabase tables | `done` — all 15 live tables live-verified for SELECT (real minted sessions via Admin API, since project uses asymmetric JWT signing keys); full write-phase live-tested on 2 tables + 2 join-table INSERT-rejection spot-checks (11 tables' writes rest on migration text, D140); consent-gate live for 2/4 states (D140); storage + function-grant audits all live incl. direct object GET; 8-layer code review complete, D138 (open) + D139 (accepted) + D140 (accepted residual scope) filed; `docs/security/rls-audit.md` committed; not yet pushed/merged | 5-3 (partial — `lesson_access`/`stripe_events` not yet on this branch) | Epic-5 DoD → W10-1 |
...
- [x] RLS audit report committed; every table's policy verified; DPDP `consent_type` gap resolved or explicitly registered — **5-5** (`docs/security/rls-audit.md`; consent_type gap registered as D138 (open, not resolved); write-path/consent-state testing scope registered as D140 rather than overclaimed)
