...
- `apps/api/app/modules/media/router.py` — a `GET /media/signed-url` endpoint is registered and has `avatar-clips` in its bucket allowlist, but the handler body is a stub: `raise HTTPException(501, "Not implemented yet")`. Nothing can actually call it yet.
  - **UPDATE (2026-07-29, found during Story 1-5's implementation/review):** both claims above are now stale. The handler is fully implemented (no longer a 501 stub), and `avatar-clips` was **removed** from `_ALLOWED_BUCKETS` by Story 2-25 — its flat clip paths (`clips/intro_default.mp4`, no UUID prefix) always 404 against this endpoint's `{lesson_id}/...` ownership check, so it was disabled rather than fixed. The "open question" below about signed-URL expiry vs. a public bucket is consequently more urgent than originally framed: the signed-URL path isn't just risky, it's currently non-functional for this bucket. Whoever resumes this work needs to re-enable a corrected allowlist entry (or make the bucket public) as a prerequisite, not just answer the expiry question.
