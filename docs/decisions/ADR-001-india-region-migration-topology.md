...
**Status:** 🟢 **IMPLEMENTED 2026-08-14 — provider chosen (Fly.io), topology and Langfuse as decided below.** One open follow-up: **D145** (`docs/DEFECT-REGISTER.md`) — the live region list has been observed drifted to `sin` (Singapore) instead of this ADR's `bom` (Mumbai) intent; registered, not fixed. See also **D164** (CLAUDE.md's stack table was stale about this for three weeks after implementation).
