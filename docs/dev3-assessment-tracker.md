...
**Last updated:** 2026-09-05 (Story 4-33 done — Learner DNA profile quality review script; Sprint 4 now 10/11)
...
- [x] **Update tuned weights in Railway env vars (Story 4-32)** — ✓ 2026-09-04
  ...
  - **Status (done 2026-09-04):** `docs/sprint4-railway-env-update.md` operator runbook written (weights table, Railway steps, CLI alternative, rollback values). `apps/api/scripts/verify_ces_weights.py` standalone verification script (16 tests, 36/36 pass). Branch: `sprint4/s4-32-railway-env-vars`

- [x] **Learner DNA profile quality review (Story 4-33)** — ✓ 2026-09-05
  - Extract 10 real `learner_dna.profile_text` values
  - Review checklist per profile: no clinical claims, no raw numbers, DPDP disclaimer present, tone is encouraging, plain badge labels
  - Document any failing profiles and the prompt fix applied
  - **AC:** All 5 rules implemented; 10 sample profiles reviewed; 39/39 tests PASS; results in `docs/sprint4-dna-quality-review.md`
  - **Status (done 2026-09-05):** `apps/api/scripts/review_dna_profiles.py` standalone review script (5 rules, fixture + Supabase modes, signals_capped warning). `apps/api/tests/fixtures/sample_dna_profiles.json` (10 profiles: 5 pass, 5 with intentional violations). `apps/api/tests/test_s4_33_dna_review.py` (39 tests, 39/39 GREEN). `docs/sprint4-dna-quality-review.md` with results + prompt-fix recommendations. Branch: `sprint4/s4-33-dna-profile-quality-review`
