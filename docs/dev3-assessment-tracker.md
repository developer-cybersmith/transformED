...
**Last updated:** 2026-08-13 (Demo Sprint tasks added from HIE-Demo-Task-Tracker.xlsx — T15/T16/T18/T19/T20/T26/T28)
...
## Demo Sprint — End-to-End Demo Readiness

> **Goal:** Validate the full lesson-to-assessment pipeline against the first real generated packages. Freeze the CES wire contract (SYNC-A / SYNC-B), resolve dead CES implementation debt, and run the full end-to-end success test with a real student.
>
> Source: `d:/HIE-Demo-Task-Tracker.xlsx` (28 total tasks across all devs, phases L0–L8 + Cross-team). Dev 3 owns or co-owns 7 of these.

### Individual Dev 3 Tasks

- [ ] **T18 (L5) — Expose quiz_accuracy / teach-back fractions on 0-1 scale**
  - **Owner:** Dev 3 | **Priority:** High | **Blocking:** Contract freeze SYNC-A | **Status:** Not started
  - `ces_contribution` is already weight-multiplied — **do not reuse it directly.** The wire contract expects raw 0-1 fractions: `quiz_accuracy = correct_count / total_questions`, `teachback_fraction = raw_score / 100.0`. Must be computed and exposed separately from the CES formula output.

- [ ] **T28 (Cross-team) — Delete dead CES implementation + validate scoring against first real package**
  - **Owner:** Dev 3 | **Priority:** High | **Blocking:** Contract freeze SYNC-A
  - Remove any stale/superseded CES computation code that predates the `ces.py` formula. Run `POST /api/assessment/quiz` and `POST /api/assessment/teachback` against the first real `LessonPackage` (not fixtures) and confirm session rows are created and scoring is correct end-to-end.

### Collaborative Tasks (Dev 3 is co-owner)

- [x] **T15 (L4) — Validate quiz + teach-back payloads against first real package** — ✓ 2026-08-13
  - **Owner:** Dev 3 + Dev 2 | **Priority:** High | **Blocked by:** L1 (real package available)
  - Confirm a real session row is created and both endpoints (`/quiz`, `/teachback`) find it. Not against fixtures — must use the LessonPackage generated by Dev 1's pipeline.

- [x] **T16 (L5) — CES v1 wire contract freeze (SYNC-A) — Dev 3 + Dev 4 sync** — ✓ 2026-08-13
  - **Owner:** Dev 3 + Dev 4 | **Priority:** Critical | **Blocking:** Contract freeze SYNC-A
  - Agreed wire contract between CES formula (Dev 3) and WebSocket signal producer (Dev 4). **Note:** Testing is still remaining for all 19 CES scenarios (see `docs/stories/3-54-*` coverage matrix). Implementation marked done; test coverage is the open item.

- [ ] **T19 (L5) — Decide whether a partial CES signal is valid** [IN PROGRESS]
  - **Owner:** Dev 3 + Dev 4 | **Priority:** High | **Blocking:** SYNC-B; blocks L6
  - Question: if one or more of the 3 MediaPipe-derived fields (head_pose, blink, behavioral) is missing/None, should CES still fire or be suppressed? Dev 3 owns the formula logic; Dev 4 owns the signal producer. Decision must be recorded in a story or ADR before implementation.

- [ ] **T20 (L5) — Freeze attention_signal wire contract with explicit field ranges in ws.ts**
  - **Owner:** Dev 2 + Dev 3 + Dev 4 | **Priority:** Critical | **Blocking:** SYNC-B; blocks L6
  - `ws.ts` currently declares every field as bare `number` with no range. Must add explicit ranges (e.g., `head_pose: number // 0.0–1.0`, `blink_rate: number // 0–50/min`) so all three consumers (Dev 2 renderer, Dev 3 CES formula, Dev 4 state machine) agree on normalisation. Requires 4-dev PR review (frozen contract).

- [ ] **T26 (L8) — Full end-to-end sprint success test with one real student, unassisted**
  - **Owner:** All devs | **Priority:** Critical | **Blocked by:** L0–L7 complete
  - The first-ever measurement of: cost/lesson, wall-clock generation time, audio/slide drift, narration length cap enforcement, truncation warnings, CES formula agreement against real session data. Dev 3's exit numbers: session report non-null, CES non-zero, quiz/teachback scores present, duration correct.
