...
- As a **student**, if I look away from the screen for too long, the lesson gently prompts me to re-engage â€” the lesson does not pause on its own.
...
- As a **platform operator**, no GPT call is made at intervention time â€” all messages are pre-baked at lesson build time.
...
### Endpoint
`/ws/{session_id}` â€” accepts connection after JWT validation in handshake headers.
...
- `broadcast(session_id, message)` â€” sends JSON message to connected client
- `disconnect(session_id)` â€” removes from map, cleans up Redis cooldown keys
...
| `session_end` | `{ report_url: str }` | TEACH_BACK â†’ SESSION_END |
...
```
IDLE â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â–º  (on lesson_start)
  â””â”€â–º TEACHING â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â–º  (nominal state)
        â”œâ”€â–º INTERVENING    (on distraction signal + guard rules)
        â”‚     â””â”€â–º TEACHING (on intervention_acknowledged OR timeout 30s)
        â”œâ”€â–º CHECKING_IN    (on confusion signal + guard rules)
        â”‚     â””â”€â–º TEACHING (on check_in_dismissed)
        â”œâ”€â–º QUIZZING       (on segment_boundary event)
        â”‚     â””â”€â–º TEACH_BACK (on quiz_complete)
        â”‚           â””â”€â–º TEACHING (on teachback_end, if not last segment)
        â”‚           â””â”€â–º SESSION_END (on teachback_end, if last segment)
        â””â”€â–º SESSION_END    (on lesson_end without pending quiz)
```
...
|---|---|---|
| A â€” Distraction | `head_pose_score < 0.4` for 2 consecutive windows | `intervention_messages` node (Epic 1) |
| B â€” Confusion | CES drops > 0.15 in one 30s window | `intervention_messages` node (Epic 1) |
| C â€” Fatigue | Session duration > 40 min AND blink_rate elevated | `intervention_messages` node (Epic 1) |
...
| DB migrations | `supabase/migrations/` â€” `session_events`, `tutor_state_log` |
...
| Epic 2: WebSocket client + AttentionMonitor sending signals | Parallel â€” interface contract agreed Sprint 1 |
...
- [ ] JWT decoded locally with PyJWT â€” no remote call (verified by unit test with no network)
...
- [ ] Redis writes are non-blocking â€” WebSocket handler does not await signal buffer write
- [ ] `ces_update` messages emitted to client at 5-second intervals during TEACHING
- [ ] All intervention messages sourced from `lesson_package.json` â€” no inline strings, no GPT calls
