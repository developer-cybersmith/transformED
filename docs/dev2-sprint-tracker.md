...
**confirmed capture-and-log only for v1** (no live AI Q&A backend exists anywhere in this codebase;
registered as **D149**, handed off to Dev 3 with a documented API contract, frontend built against
a stub mirroring `payment.service.ts::checkAccess`'s exact D136 pattern). Two real implementation
