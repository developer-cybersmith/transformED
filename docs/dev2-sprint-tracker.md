...
**confirmed capture-and-log only for v1** (registered as **D159** — renumbered from this story's
original D149, which collided with the Sarvam batching fix that merged to `main` while this PR was
open; a real backend has since landed separately as D158/Story 4-28, not yet wired to this stub).
Frontend built against a stub mirroring `payment.service.ts::checkAccess`'s exact D136 pattern.
