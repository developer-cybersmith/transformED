...
| **Last Updated** | 2026-09-08 (**BR-2 (Mode-Selection Cards: Explicit Duration Labels, Story 2-60) DONE.** BR-2's own stated dependency (Dev 3 confirming the tier→minute mapping) was already resolved via Dev 3's Story F2-3 (2026-09-04), so this was a pure frontend addition: `durationMinutes` added to `LearnerTierOption` (`deep: 45, balanced: 30, refresher: 15`, matching `_TIER_MINUTES` via the already-existing `LEARNER_TIER_TO_BACKEND` mapping — no new backend call), rendered as a small pill badge next to each card's existing name in `ModeSelection.tsx` (name kept as-is, alongside not instead of). 3 new tests, all 11 pre-existing tests pass unmodified, full frontend suite 93 files/1149 tests green. See `docs/stories/2-60-mode-selection-duration-labels.md`.) — previously 2026-09-07 (**BR-8 (Reports Pages UI/UX Redesign, Story 2-59) DONE — ad-hoc, direct user feedback right after BR-7 shipped.** `SessionReport.tsx` and `ReportsIndex.tsx` redesigned (rebalanced content region, recency-grouped filter tabs + pagination); found and fixed **D161** (SSR/CSR locale hydration mismatch) same day. **Same-day BR-5 follow-up**: live re-verification (sub-100ms `aria-label` polling, not periodic snapshots) confirmed the slide-transition pause fires correctly but was too easy to miss — fixed via PR #213 (5s duration, new visibility pill, persistent Dashboard link on the player). See §13A for both. **Cross-team, explicit user-approved backend exceptions this same session**: found and fixed three real production defects while capturing UI screenshots for a user manual — **D163** (every lesson-start session-creation call 500ing since Story 4-13, two compounding bugs), **D164** (Ask-Tutor's Redis rate-limit check failing open instead of closed under D162's Upstash quota exhaustion), **D165** (`CLAUDE.md`'s stack table stale about the Railway→Fly.io/Upstash migration for three weeks) — all registered in `docs/DEFECT-REGISTER.md`, fixed via PRs #212/#214/#209. Also reviewed and fixed real issues across 6 of Dev 3's/Dev 1's own PRs this session (#207, #208, #209, #216, #217, #218) plus one of Dev 4's (#163) — a stale citation, a stale blocker reference, a story-status/tracker mismatch, and a genuine 3-way tracker merge collision across #216/#217/#218, all found and fixed before merge, not after. Root-level `.md` housekeeping this same pass: `D132-FIX-TRACKER.md`/`D133-FIX-TRACKER.md`/`RUN-FINDINGS-LOG.md` moved to `docs/fixes/`, `SPRINT-3-STATUS-REPORT.md` and this session's own `sprint4-dev2-report.md` moved to `docs/reports/`, every cross-reference updated; `CLAUDE.md`/`DEV1-FIX-PLAN.md` deliberately left at root (framework convention / too heavily cross-referenced by story provenance to move safely).) — previously 2026-09-05 (**BR-7 (Reports Index Page, Story 2-58) DONE — ad-hoc, not one of the original 6 Bug Resolution items.** User-reported: `Sidebar.tsx`'s "Reports" nav link has pointed at `/reports` since the sidebar was first built, 404ing from the beginning — only `/reports/[sessionId]` (a single session's report) ever existed, reachable only from `Player.tsx`'s post-lesson "View Session Report" link, with no way back to any past report once the student navigated away. Full-stack fix, explicitly approved by the user after being asked (no backend list-sessions endpoint existed, and this session does not touch `apps/api` without explicit go-ahead): new `GET /assessment/sessions` (`schemas.py::SessionSummary`, `service.py::list_sessions`, `router.py`) returns the caller's own sessions, most recent first, bounded to 50 rows (`.limit()`, real ownership scoping via explicit `.eq("user_id", ...)` since the service-role Supabase client bypasses RLS) and reuses `get_session_report`'s own tier-lookup/ownership pattern plus `admin/router.py`'s established `lessons(...)` PostgREST-embed convention. New `apps/web/src/app/reports/page.tsx` + `ReportsIndex.tsx` (loading/error/empty/list states) + `useSessionReports()` hook (mirrors `useSessionReport.ts`'s SWR pattern) + `listSessions()`/`SessionSummary` in `lib/assessment.ts`/`types/assessment.ts`. **Verified live, not just by test**: temporarily added `/reports` to `proxy.ts`'s auth deny-list bypass on the local dev server only (immediately reverted, confirmed zero diff before committing) to get past the unauthenticated-redirect ambiguity — a genuinely-missing route and an auth-redirected one both 307 to `/signin`, so this was the only way to prove the route itself resolves — and confirmed a real `200` serving `reports/page.tsx` with the actual component markup, not Next's 404 page. 11 new backend tests (`test_list_sessions_endpoint.py`) + 9 new frontend tests, full backend targeted suite 109/109 and full frontend suite 91 files/1091 tests green, zero regressions. Story-first BMAD flow followed: dedicated branch (`bug-resolution/br-7-reports-index-page`), story-only commit pushed before any code. See `docs/stories/2-58-reports-index-page.md`.) — previously 2026-09-03 (**BR-5 (Configurable Slide-Transition Pause + Manual Next Button, Story 2-57) DONE.** Disambiguated with the user first (within-segment slide swap, not segment-to-segment advancement). Reused `PlayerStatus.PAUSED` + a new `pauseReason` field rather than a parallel mechanism. Two additions folded in after initial scoping: a per-segment "skip pause" checkbox, and an "Ask Tutor" button, originally capture-and-log only for v1 (registered **D159**, renumbered from this story's original D149 which collided with an unrelated Sarvam fix merged in the meantime) but wired to the real backend (D158/Story 4-28) in this same pre-merge pass — `submitTutorQuestion()` now calls the real endpoint and `AskTutorPanel.tsx` renders the returned answer or a graceful decline, closing D159. Two real bugs found and fixed during implementation: a quiz-firing regression when a slide boundary coincided with the segment end, and an Ask-Tutor guard that blocked asking a question while already auto-paused. Full suite 91/1118 at initial implementation (+3 tests added wiring D159 to the real backend), zero regressions. See `docs/stories/2-57-slide-transition-pause.md`.) — previously 2026-09-03 (**S4-05 (Performance: Code Splitting + Lazy Loading, Story 2-56) DONE.** Grounded research found 2 of 4 backlog bullets already true (MediaPipe lazy-loading, verified not re-implemented), 1 genuinely fixed (`AttentionChart`/`recharts`, a real ~390KB chunk, converted from a static import — eagerly loaded on every `/reports/[sessionId]` visit regardless of whether the chart ever rendered — to `next/dynamic(..., {ssr:false})`; verified via two clean `next build` runs that the chunk moved from the route's eager `entryJSFiles` to `react-loadable-manifest.json`), 1 dropped as moot (HeyGen preload — HeyGen removed entirely per D144). Lighthouse `/lesson/[id]` > 70 target honestly scoped down: no test credentials exist to reach a real authenticated lesson page (confirmed live, 307 → `/signin`); verified the tooling itself works via a real run against `/` (73/100) instead of fabricating a score for the actual target route — flagged as a residual gap needing a disposable test account. Full suite 90 files/1088 tests green, zero regressions. See `docs/stories/2-56-performance-code-splitting.md`.) — previously 2026-09-01 (**Sprint 4 fully landed on `main` + Bug Resolution Sprint (Feature Sprint 2) added.** S4-02/S4-03/S4-04 and the cross-team `attention_signal` null-vs-0.0 fix all merged from `sprint4-master` into `main` via PR #161. Reviewed and merged 5 of Dev 3's own Sprint-4-adjacent PRs (#164 session dedup, #165 auth 401 fix, #166 onboarding audit + session hardening, #167 D137 reassessment-blend fix, #168 Learner-DNA-personalized CES threshold) — found and got fixed, in cross-team review rather than after merge: (1) #166's S4-5 onboarding reword silently corrupted Learner DNA scoring on 7 questions (position-based scoring, not content-based — a reorder-for-clarity pass scrambled the intended gradient); (2) reassessment (D137) was discarding 10 sessions of real behavioral EMA data on every retake instead of blending; (3) #168's new CES-threshold formula had `frustration_tolerance`'s sign inverted relative to its real, established meaning in `dna_fusion.py`, plus two CI guard-test regressions hidden in an "advisory, not gating" bucket that doesn't fail the build — caught by pulling the real CI log instead of trusting the green checkmark, which is now a standing CLAUDE.md rule. **New §13A added**: Bug Resolution Sprint (Feature Sprint 2), 6 tasks (BR-1 through BR-6), started immediately post-Sprint-4 on per-dev branches — none of Dev 2's own started yet; flagged a real cross-team inconsistency in who owns the voice-teach-back STT node (BR-6) before assuming Dev 1.) — previously 2026-08-29 (**sprint4-master reconciliation: S4-03 and S4-04 merged together, resolving a real conflict in this file's header/dashboard rows since both task branches had independently updated them from the same base.** S4-04 (Accessibility Audit WCAG AA, Story 2-55) shipped: shared `useRovingRadioGroup` hook (roving tabindex + wrapping arrow-key nav) added to `QuizOverlay.tsx`/`QuestionCard.tsx`; `role="status" aria-live="polite"` added to `QuizOverlay`'s quiz feedback and split into a permanently-mounted announcer + the existing visual toast for `TutorInterventionCard` (8-agent review found a keyed remount-per-intervention node risked a screen reader missing the announcement); focus-visible rings deduplicated into `lib/a11y/focusRing.ts`; `text-neutral-400`→`text-neutral-600` contrast fix at 3 sites; alt text confirmed compliant. See `docs/stories/2-55-accessibility-audit-wcag-aa.md`.) — previously 2026-08-27 (**Sprint 4 tracker sync + backfill.** S4-02 (Razorpay Checkout Frontend, Story 2-53): `RazorpayCheckoutButton`/`useRazorpayCheckout`/`payment.service.ts` built, 8-agent review passed, merged into `sprint4-master` — but marked PARTIAL, not DONE, since the component isn't wired into any real page yet and the backend's `GET /api/payments/access` doesn't exist yet (frontend polls a documented mock, D136). **S4-03 (PostHog Full Instrumentation, Story 2-54) DONE**: all 8 events wired, 8-agent review found and fixed a real cross-lesson data-corruption bug plus a reference-vs-content-hash dedup bug plus an autocapture volume/privacy gap; same-day fast-follow wired `posthog.identify()`/`reset()` into `AuthContext.tsx` so events tie to real accounts, not anonymous browser IDs — still needs the PostHog key added to Vercel's production env before it produces real data. **Backfilled S4-10/S4-11/S4-12 into this tracker** (ad-hoc numbers assigned during story creation this sprint, never previously given headers here): S4-10 (loading/error/empty states, Story 2-50) DONE, S4-11 (lesson poll ceiling + error boundary, Story 2-51) DONE, S4-12 (email notifications, Story 2-52) DONE — all 3 already merged into `sprint4-master` with 8-agent reviews passed. Also removed a stray leftover git merge-conflict marker (`>>>>>>> origin/sprint4/s4-07-caption-overlay-scroll-fix`) found in §13 during this pass — real corruption, unrelated to this update, fixed while here. §1 dashboard Sprint 4 row and Total row both recomputed.) — previously 2026-08-17 (**S4-09 CaptionOverlay redesigned to one-line-at-a-time captions** — direct user feedback after S4-07: no word-level TTS timing exists, so lines are split ~10 words at a time and timed proportionally to character count within the segment's known duration; verified live advancing correctly during real playback. See §13.) — previously 2026-08-17 (**S4-08 lesson-restart-after-teachback fix shipped** — user-reported live bug: `exitTeachBack()` resuming PLAYING on an already-ended last-segment `<audio>` element replayed it from 0 per the HTML media spec, indistinguishable from the lesson restarting. See §13.) — previously 2026-08-17 (**S4-07 CaptionOverlay unreachable-scroll fix shipped** — real live production bug found via manual Playwright browser verification, `pointer-events-none` + missing `data-lenis-prevent` stacked to make the caption panel's own overflow scroll completely unreachable. See §13.) — previously 2026-08-17 (**S4-06 Fold My Library into My Books shipped** — ad-hoc off a live product decision, not scheduled Sprint 4 scope; full BMAD flow with 6-agent review, see §13. Backend touches Dev 1's `content/router.py`/`schemas.py` under an explicit user-approved exception.) — previously 2026-08-11 (**S3-09 Signed-URL auto-refresh + DEFER-012 (D63) register entry shipped** — Story 2-45, ad-hoc off the Lesson Delivery Tracker's L3 known risk, not a scheduled Sprint 3 item; branch cut from `sprint3-master` since `main` doesn't yet have S3-01–S3-04. `AudioTimeline`/`SlideRenderer` now auto re-sign an expired asset once before falling back to the existing manual Retry/placeholder path. 920 web tests passing. See §12.) — previously 2026-08-06 (**S3-01 Attention Consent Modal shipped** — Story 2-42, 3-agent review passed, all 3 decision-needed + 12 patch findings resolved; switched to the real `POST /api/assessment/consent` endpoint (Story 3-32) after it landed on `main` mid-review, closing **D29**. **S3-04 CES Indicator confirmed shipped** — Story 2-41 (this file had drifted; it was done alongside S3-03 but never updated below). `sprint3-master` synced with `main` (also picked up **D18**/**D30** closures) and pushed. Sprint 3 is now 3/10 done — see §12.) — previously 2026-08-03 (S3-03 TutorInterventionCard shipped — Story 2-40, 3-agent review passed, 9 patches applied. See §12.) — previously 2026-07-29 (D27: `/signin` Suspense-boundary fix shipped — the app's first-ever successful production build. **S2-34** (browser SpeechSynthesis fallback, last tier of the TTS fallback chain) shipped: story-first commit, TDD implementation, 3-agent adversarial code review, 7 patches applied. **PR #114 merged `sprint2-master` into `main`** — S2-11 through S2-15, S2-34, and this file's own tracker updates are now all on `main` alongside the previously-direct-to-`main` S2-26/S2-33. `main` and `sprint2-master` are fully in sync as of this merge. **Cross-team Sprint 2 completion audit run same day — see note below; Dev 2's own scope is done, but the assessment path is blocked cross-team by D18.**) |
| **Active Sprint** | Bug Resolution (Feature Sprint 2) — Post-Sprint 4 — **IN PROGRESS, 4/8 done** (BR-2 done 2026-09-08, Story 2-60; BR-5 done 2026-09-03, Story 2-57, follow-up shipped 2026-09-07 (5s duration + visibility pill, PR #213) after direct user feedback; BR-7 done 2026-09-05, ad-hoc — see header note; BR-8 done 2026-09-07, ad-hoc — Reports UI/UX redesign, Story 2-59, see header note; BR-6 blocked pending STT-node-ownership clarification; BR-3/BR-4 blocked on Dev 1's caption-timestamp output — message sent 2026-09-03; BR-1 not started). Sprint 4 — Weeks 8–9 — **DONE on `main`, 10 done / 1 partial / 1 not started** (of 12, +7 ad-hoc: S4-06 through S4-12) — S4-02/03/04 confirmed merged all the way to `main` (not just `sprint4-master`) via PR #161. Not-started: S4-01 (Pricing page — pricing content itself on hold). Partial: S4-02 (Razorpay frontend built and merged to `main`, not yet page-integrated, still blocked on Dev 1's backend PR #157 which has a failing CI check). Sprint 3 — Weeks 6–7 — **COMPLETE, 9/9** (S3-01 through S3-09, closed 2026-08-19 via Story 2-49/PR #149). **note:** this row's "Total Tasks" in §1 has read 10 since before an earlier correction, and 9 S3-numbered items (S3-01–S3-09) exist in this file after S3-09 was added as an ad-hoc item — a pre-existing count discrepancy, not fully resolved by this update, left flagged rather than silently resolved) |
| **Overall Status** | Sprint 0 COMPLETE · Sprint 1 COMPLETE (14/14) · Sprint 2 (Dev 2 scope) COMPLETE (10/10 + 8 additional stories) — **cross-team Sprint 2 is now end-to-end functional; D18/D29/D30 all closed 2026-08-04/05** · Sprint 3 **COMPLETE (9/9)** — S3-01 through S3-09 all done, S3-06 closed 2026-08-18 via Story 2-48, S3-08 (Mobile Responsive Audit, the last remaining item) closed 2026-08-19 via Story 2-49 — **corrected 2026-08-11: S3-09 (signed-URL auto-refresh + DEFER-012/D63) added and shipped same day, ad-hoc off the Lesson Delivery Tracker rather than originally-scheduled Sprint 3 scope** · Sprint 4 **IN PROGRESS (10 done / 1 partial / 1 not started of 12+7 ad-hoc)** — S4-06 through S4-12 all done (S4-06–09 shipped 2026-08-17, S4-10/11/12 shipped 2026-08-26, S4-03 shipped 2026-08-27, S4-04 shipped 2026-08-29, S4-05 shipped 2026-09-03 via Story 2-56); S4-02 partial (frontend unit built and reviewed 2026-08-27, not yet page-integrated); S4-01 not started (pricing content on hold); **all of Sprint 4's Dev 2 work confirmed merged to `main` via PR #161, 2026-09-01, not just `sprint4-master`** · **Bug Resolution (Feature Sprint 2) IN PROGRESS (4/8 for Dev 2)** — BR-2 done 2026-09-08, BR-5 done 2026-09-03 (follow-up 2026-09-07), BR-7 done 2026-09-05, BR-8 done 2026-09-07 (both ad-hoc); started immediately post-Sprint-4 on per-dev branches |
...
| Sprint 4 | Weeks 8–9 | 12 (+7 ad-hoc: S4-06 through S4-12) | **10** | **1** | **1** |
| Bug Resolution (Feature Sprint 2) | Post-Sprint 4 | 8 (BR-1 through BR-6, +2 ad-hoc: BR-7, BR-8) | **4** | 0 | **4** |
| Launch | Week 10 | 5 | 0 | 0 | **5** |
| **Total** | **10 weeks + Bug Resolution** | **68** | **46** | **1** | **21** |
...
### S3-06 — Reports Page — ✓ DONE 2026-08-18
**Priority:** P1  
**Status:** ✅ DONE 2026-08-18. Attention timeline chart shipped separately as Story 2-46/S3-05. Teach-back summary detail shipped as Story 2-48 — backend (PR #145, Dev 3) and frontend (PR #146, Dev 2) both merged into `main`.  
...
Teach-back summary detail: Story `2-48-teachback-summary-detail.md`. The real report endpoint already read `teachback_attempts` but only selected the aggregate `score` — the richer per-attempt columns (`feedback_praise`, `feedback_correction`, `concepts_hit`, `concepts_missed`) were already persisted and just needed exposing. Per the team-boundary rule (Dev 2 does not touch `apps/api`), the backend extension was spec'd in `docs/handoffs/dev2-to-dev3-teachback-detail-handoff-2026-08-18.md` and implemented by Dev 3 (PR #145, `apps/api/app/modules/assessment/{router,service}.py` — new `TeachbackDetail` model, widened `.select()`, `.order("created_at")`). Frontend (`types/assessment.ts`'s `TeachbackDetail`, `SessionReport.tsx`'s new `TeachbackDetailSection`) built against that contract, then corrected post-merge (`score` widened to `number | null` to match the real shipped shape) in PR #146. A branch-name collision between the two halves (both used `sprint3/s3-06-teachback-detail`) forced the frontend to re-push as `sprint3/s3-06-teachback-detail-fe`; the two independently-written story files were reconciled into one canonical `docs/stories/2-48-teachback-summary-detail.md` after both merged. Note: "quiz accuracy by segment" remains explicitly not buildable as scoped (see S2-04 Dev Notes) — separate from this item, still needs its own Dev 3 endpoint if ever prioritized.
...
### S3-08 — Mobile Responsive Audit — ✓ DONE 2026-08-19
**Priority:** P2  
**Status:** ✅ DONE 2026-08-19. Story `2-49-mobile-responsive-audit.md`, branch `sprint3/s3-08-mobile-responsive-audit`, merged via PR #149. Pre-implementation research found the real gap narrower than "audit everything": the dashboard shell's mobile nav already existed and was already tested, dashboard/books already had real responsive classes — the actual gaps were the player having zero mobile handling and two genuine, previously-unknown horizontal-overflow bugs.  
**Files:** `src/components/player/MobileNotice.tsx` (new), `src/components/player/Player.tsx`, `src/components/reports/SessionReport.tsx`, `src/app/pending-approval/page.tsx`, `src/test/setup.ts` (new global `matchMedia` polyfill).

Built the player's "Desktop recommended" banner (`MobileNotice.tsx`, below the `md` breakpoint, dismissible, never gates playback) — the tracker's own explicit ask, which had zero prior implementation. Fixed two real bugs found during live Playwright verification against a real dev server + backend: `SessionReport.tsx`'s three root containers were all missing horizontal padding (content sat flush against the screen edge on mobile), and `/pending-approval` was genuinely horizontally scrollable at 768px (a decorative background div missing the same `overflow-hidden` its sibling `signin`/`signup` pages already had). `PlayerControls`/`JargonHover` — flagged as unverified overflow risks by the pre-implementation research — were both confirmed live to NOT be broken, so no fix was applied to either (verified before fixing, not assumed). Verified live at 375/768/1024px plus two real device ratios (iPad Air 820×1180, phone landscape 812×375) across every Dev-2-owned route, including a full real lesson completed end-to-end to reach and verify the Reports success state. `/onboarding` explicitly not verified — gated behind a new-user account state not reachable with the available test account; recorded as an open gap, not silently skipped. 80 files/984 tests green, `tsc`/`eslint` clean.
...
**Status:** 🔵 PARTIAL — ✅ 2026-08-27 (Story 2-53, 8-agent review passed, merged into `sprint4-master`): `RazorpayCheckoutButton.tsx` + `useRazorpayCheckout` state machine + `payment.service.ts` built, tested, reviewed. **Not yet wired into any real page/route** — the component exists standalone but nothing in `apps/web/src/app` imports it yet, and the backend's `GET /api/payments/access` doesn't exist yet either (frontend polls a documented mock, D136). Both gaps are tracked, not silently dropped. **provider changed from Stripe to Razorpay 2026-08-24, see `docs/decisions/ADR-002-payment-gateway-razorpay.md`**  
<!-- STATUS-DETAIL: frontend checkout unit complete, not yet integrated into a page; blocked on backend GET /api/payments/access -->
**Files created:** `src/components/payment/RazorpayCheckoutButton.tsx`, `src/hooks/useRazorpayCheckout.ts`, `src/services/payment.service.ts`, `src/types/payment.ts`
...
**Priority:** P1  
**Status:** ✅ DONE — ✅ 2026-08-27 (Story 2-54, 8-agent review passed, merged into `sprint4-master`)  
<!-- completed: 2026-08-27 -->
All 8 events wired at real, code-verified trigger points (`onboarding_completed`, `upload_started`, `upload_completed`, `lesson_started`, `lesson_completed`, `quiz_answered`, `teachback_submitted`, `intervention_received`) via `apps/web/src/instrumentation-client.ts`. Review round found and fixed a real cross-lesson data-corruption bug (stale player-store state on a fresh lesson mount), a reference-vs-content-hash dedup bug, and an uncontrolled-autocapture volume/privacy issue. Same-day fast-follow: `posthog.identify()`/`reset()` wired into `AuthContext.tsx` so events tie to the real account, not an anonymous per-browser ID. **Still needed before this produces real data**: `NEXT_PUBLIC_POSTHOG_KEY`/`HOST` added to Vercel's production env vars (currently local-dev-only).  
...
**Priority:** P1  
**Status:** ✅ DONE (2026-08-29) — Story 2-55, branch `sprint4/s4-04-accessibility-audit`. All 5 checklist items addressed: shared `useRovingRadioGroup` hook (arrow-key nav + roving tabindex) wired into `QuizOverlay.tsx` and `QuestionCard.tsx` (the latter had pre-existing ARIA radiogroup roles with no keyboard behavior behind them — a real, pre-existing bug fixed here, not introduced by this story); `role="status" aria-live="polite"` on `QuizOverlay`'s quiz feedback and `TutorInterventionCard` (the latter split into a permanently-mounted announcer + the existing remount-per-intervention visual toast, so the live-region announcement survives a screen reader's aria-live semantics reliably); focus-visible rings added to every raw `<button>` across `QuizOverlay.tsx`/`TutorInterventionCard.tsx`/`TeachBackModal.tsx`; `text-neutral-400` → `text-neutral-600` contrast fix (3 confirmed sites); alt text confirmed already compliant (11 `<img>`/`<Image>` sites, all with real or intentionally-decorative alt). 8-agent adversarial review round run; all findings fixed in-branch. Full suite/lint/typecheck/build green.  
...
**Priority:** P2  
**Status:** ✅ DONE — ✅ 2026-09-03 (Story 2-56, branch `sprint4/s4-05-code-splitting-lazy-loading`)  
Grounded research found this bullet list was aspirational backlog text, not a status check against
the real code — two of its four items were already true, one needed a real fix, one is moot:
- MediaPipe WASM bundle lazy-loading — **already done, verified not implemented**:
  `useAttentionMonitor.ts` already dynamically `import()`s `@mediapipe/tasks-vision`, gated on
  `consentStatus === 'accepted' && hasReachedTeaching`. No static import exists anywhere.
- Chart library dynamic import — **genuinely fixed**. `AttentionChart`/`recharts` (a real ~390KB
  chunk) was statically imported into `SessionReport.tsx` and eagerly loaded on every
  `/reports/[sessionId]` visit regardless of whether the chart ever rendered — confirmed via a real
  `next build`'s client-reference-manifest. Converted to `next/dynamic(..., { ssr: false })`,
  mirroring `PlayerLoader.tsx`'s existing pattern. Re-verified via a second clean build: the chart's
  chunk is now absent from the route's eager `entryJSFiles` and present instead in
  `react-loadable-manifest.json` — the exact before/after signature of a real fix, not just a claim.
- HeyGen video preload — **dropped as moot**. HeyGen was removed entirely as dead code (D144,
  2026-09-02) — nothing left to preload.
- Lighthouse `/lesson/[id]` > 70 — **honestly scoped, not fully closeable this pass**. No test
  credentials exist to reach a real authenticated `/lesson/[id]` (confirmed live: unauthenticated
  request 307s to `/signin`). Verified the tooling itself works with a real run against the
  reachable `/` route instead (73/100, real measured Lighthouse output) — the actual `/lesson/[id]`
  score remains unverified and is flagged as a residual gap requiring a disposable test account.
Full detail, evidence, and file list in `docs/stories/2-56-performance-code-splitting.md`. Full
frontend suite (90 files / 1088 tests) green, zero regressions; `tsc --noEmit`/`eslint` clean.
...
### S4-10 — Loading + Error + Empty States for All Flows
**Priority:** P1 — matches `docs/master-tracker.md`'s Dev 2 Sprint 4 line "Loading + error + empty states for all flows"  
**Status:** ✅ DONE — ✅ 2026-08-26 (Story 2-50, 8-agent review passed, merged into `sprint4-master`)  
<!-- completed: 2026-08-26 -->
Ad-hoc S4-number assigned during story creation (no prior entry existed in this tracker). Loading/error/empty states added across settings tabs and dashboard flows. Full details in `docs/stories/2-50-loading-error-empty-states.md`.

### S4-11 — Lesson-Status Poll Ceiling + Lesson Route Error Boundary
**Priority:** P1 — hardening ahead of real student load  
**Status:** ✅ DONE — ✅ 2026-08-26 (Story 2-51, 8-agent review passed, merged into `sprint4-master`)  
<!-- completed: 2026-08-26 -->
Ad-hoc S4-number assigned during story creation (no prior entry existed in this tracker or `docs/master-tracker.md`). Adds an explicit poll-timeout UX state (was previously silent-forever polling) and a route-level error boundary for `/lesson/[id]`. Full details in `docs/stories/2-51-lesson-poll-ceiling-error-boundary.md`.

### S4-12 — Email Notifications (Lesson Ready, Session Report)
**Priority:** P0 — reprioritized ahead of S4-02 (Razorpay) by explicit user instruction, 2026-08-25  
**Status:** ✅ DONE — ✅ 2026-08-26 (Story 2-52, 8-agent review passed, merged into `sprint4-master`)  
<!-- completed: 2026-08-26 -->
Matches `docs/master-tracker.md`'s Dev 2 Sprint 4 line "Email notifications (lesson ready, session report)". Backend feature (crosses into `apps/api`, explicit user-approved exception): `notification_log` migration, `EmailProvider`/`ResendEmailProvider` abstraction (Resend), ARQ job with an atomic idempotency claim, wired into `content_pipeline_job` and `session_end_node`. Review round found and fixed a critical claim-before-send bug that could permanently and silently lose a notification. Full details in `docs/stories/2-52-email-notifications.md`.

---

## 13A. Bug Resolution Sprint (Feature Sprint 2)

Started immediately after Sprint 4 on per-dev integration branches (e.g. Dev 4's `dev4/master-bug-resolution`), rather than waiting for a full Sprint 4 close-out. 6 tasks below, as assigned to Dev 2, +1 ad-hoc (BR-7, added 2026-09-05 off a direct user report — same "ad-hoc addition" convention as Sprint 4's S4-06 through S4-12).

### BR-1 — CAPTCHA Widget on Login + Signup (reCAPTCHA v3 Invisible Badge)
**Priority:** High  
**Status:** 🔲 NOT STARTED  

reCAPTCHA v3 invisible badge on `/signin` and `/signup`. Pairs with Dev 4's backend verification task (reject low trust score before issuing a session) — coordinate on the exact token-submission contract before starting.

### BR-2 — Mode-Selection Cards: Explicit Duration Labels
**Priority:** Low
**Status:** ✅ DONE — ✅ 2026-09-08 (Story 2-60, branch `bug-resolution/br-2-mode-selection-duration-labels`)

Dependency resolved: Dev 3's Story F2-3 (done 2026-09-04) already verified and made machine-checkable
`T1=45min, T2=30min, T3=15min` (`_TIER_MINUTES`, `apps/api/app/modules/assessment/service.py`).
Added `durationMinutes` to `LearnerTierOption` (`types/learnerMode.ts`) using the already-existing
`LEARNER_TIER_TO_BACKEND` mapping — `deep: 45, balanced: 30, refresher: 15` — no new backend call,
no re-derivation. `ModeSelection.tsx` renders each card's duration as a small pill badge next to its
existing name (name kept as-is, alongside not instead of, per the task's own wording and to preserve
what "Refresher"/"Deep"/"Balanced" mean beyond just timing). 3 new tests (each card shows its own
duration, not another tier's); all 11 pre-existing tests pass unmodified. Full frontend suite
93 files/1149 tests, zero regressions. See `docs/stories/2-60-mode-selection-duration-labels.md`.
...
**Priority:** Medium  
**Status:** ✅ DONE — ✅ 2026-09-03 (Story 2-57, branch `bug-resolution/br-5-slide-transition-pause`)  
Disambiguated with the user first: this is the **within-segment** slide swap (one segment's
continuous narration silently swapping the visible slide underneath it, timestamp-boundary-driven,
zero pause before this story) — not segment-to-segment advancement, which already goes through a
quiz. Reused the existing `PlayerStatus.PAUSED` with a new `pauseReason` field rather than a
parallel pause mechanism, so every existing pause-gated effect (audio element, virtual clock,
SpeechSynthesis, quiz-boundary check) needed zero changes. **Two additions requested after the
initial scope**, both built in the same story: a "skip pause for this segment" checkbox (per-segment
opt-out, addresses the real "could feel choppy" UX risk flagged in the original scope), and an
always-available "Ask Tutor" button that pauses playback and captures a free-text question —
**originally capture-and-log only** (registered as **D159** — renumbered from this story's
original D149, which collided with the Sarvam batching fix that merged to `main` while this PR was
open) but wired to the real D158/Story 4-28 backend in this same pre-merge pass, closing D159.
Frontend was initially built against a stub mirroring `payment.service.ts::checkAccess`'s exact D136 pattern.
Two real implementation
bugs found and fixed during this story, not shipped: (1) the original design's early `return` after
the pause trigger silently broke quiz-firing whenever a slide boundary coincided with the segment's
own end — caught by the existing test suite immediately; (2) `pauseForIntervention()`'s original
guard blocked asking a question while already auto-paused at a transition — widened after writing
this story's own tests surfaced it. Full detail in `docs/stories/2-57-slide-transition-pause.md`.
Full suite: 91 files / 1118 tests (was 89/1085), zero regressions.

### BR-6 — Voice Teach-Back: Mic Capture UI
**Priority:** Medium  
**Status:** 🟡 IN PROGRESS — unblocked 2026-09-11  
...
lifted for this one endpoint. This task is now purely frontend: mic capture, recording, upload to the
real endpoint above, and a typed/voice toggle in the existing `TeachBackModal`.

### BR-7 — Reports Index Page (ad-hoc, added 2026-09-05)
**Priority:** High
**Status:** ✅ DONE — ✅ 2026-09-05 (Story 2-58, branch `bug-resolution/br-7-reports-index-page`)

Direct user report: `Sidebar.tsx`'s "Reports" nav link has pointed at `/reports` since it was first
built, 404ing from the beginning — only `/reports/[sessionId]` (a single session's report) ever
existed, reachable only from `Player.tsx`'s post-lesson "View Session Report" link. Once a student
navigated away, there was no way back to any past report at all, and no backend endpoint anywhere
listed a user's own sessions. Confirmed full-stack scope with the user before starting (asked
explicitly, since this touches `apps/api` — not done without a direct go-ahead) rather than either
silently redirecting the dead link away or fabricating a fake list against non-existent data.

New `GET /assessment/sessions` (`schemas.py::SessionSummary`, `service.py::list_sessions`,
`router.py::list_sessions_endpoint`) — bounded (`.limit(50)`), explicitly scoped to the caller's own
`user_id` (the service-role Supabase client bypasses RLS, so this is not optional), reusing
`get_session_report`'s own ownership/tier-lookup pattern and `admin/router.py`'s established
`lessons(...)` PostgREST-embed convention rather than inventing a new one. New
`apps/web/src/app/reports/page.tsx` + `ReportsIndex.tsx` (loading/error/empty/list states) +
`useSessionReports()` (mirrors `useSessionReport.ts`'s SWR pattern) + `listSessions()`/
`SessionSummary` in `lib/assessment.ts`/`types/assessment.ts`. Verified live (not just by test) by
temporarily bypassing `proxy.ts`'s auth deny-list for `/reports` on the local dev server only,
confirming a real `200` serving `reports/page.tsx`'s actual markup, then reverting immediately
(confirmed zero diff before committing) — a genuinely-missing route and an auth-redirected one are
otherwise indistinguishable by HTTP status alone (both 307 to `/signin`). 11 new backend tests +
9 new frontend tests, full backend targeted suite 109/109 and full frontend suite 91 files/1091
tests green, zero regressions. Full detail in `docs/stories/2-58-reports-index-page.md`.

### BR-8 — Reports Pages UI/UX Redesign (ad-hoc, added 2026-09-07)
**Priority:** High
**Status:** ✅ DONE — ✅ 2026-09-07 (Story 2-59, branch `bug-resolution/br-8-reports-uiux-redesign`, PR #211)

Direct user feedback right after BR-7 shipped: the Teach-Back Detail / Learner DNA Snapshot area on
`/reports/[sessionId]` had awkward empty space, and the `/reports` index page felt visually
exhausting. `SessionReport.tsx` redesigned: prominent header Focus badge, 4-across stat tiles, then
a rebalanced content region (chart full-width when present; Teach-Back Detail + Learner DNA Snapshot
paired side-by-side when both exist, single column otherwise; "Study Again" as an always-full-width
closing block). `ReportsIndex.tsx` gained recency-grouped sections, filter tabs (All/Completed/In
Progress), and "Load more" pagination, per explicit user request for all three in the same pass.
Found and fixed the same day: **D161** (SSR/CSR locale hydration mismatch on both report pages —
`toLocaleDateString`/`toLocaleString` called with no explicit locale, pinned to `'en-US'`). Full
detail in `docs/stories/2-59-reports-uiux-redesign.md`. Full frontend suite green, zero regressions.

**Same-day follow-up to BR-5** (2026-09-07, not a separate BR item): live re-verification of the
slide-transition pause (rigorous sub-100ms `aria-label` polling across a real production slide
boundary, not periodic snapshot guessing) confirmed the pause mechanism itself fires correctly, but
surfaced a real UX gap — the only visual cue was a transport-button icon swap, easy to miss, and
2 seconds was too short to notice. Fixed via PR #213: `DEFAULT_SLIDE_TRANSITION_PAUSE_MS` raised to
5000ms, and a new non-blocking visibility pill added to `Player.tsx` (mirrors the existing buffering
indicator's style) so a slide-transition pause is actually visible, not just functional. Also added,
same PR per direct user request: a persistent "Dashboard" link on the player (previously the only
