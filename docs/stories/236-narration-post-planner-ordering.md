...
## Senior Developer Review (AI) — Round 2 (independent human/agent review on PR #237)

**Review date:** 2026-09-21
**Reviewer:** `Developer-2-max` (GitHub PR comment, 10 findings)
**Outcome:** APPROVE WITH CHANGES — 7 of 10 findings confirmed real and fixed; 2 confirmed real but correctly out of this PR's scope (matches an already-accepted pre-existing pattern, or is a refactor suggestion, not a bug); 1 not a bug (a decision the reviewer wasn't aware of, already documented in this story).

Every finding was independently re-verified against the actual checked-out code before any fix was made — not accepted on the reviewer's assertion alone (same discipline as Round 1).

### Findings — confirmed and fixed

| # | Finding | Verification | Fix |
|---|---|---|---|
| 1 | **Cost-ceiling gate full-aborted the whole lesson AFTER premium spend** (`_fan_out_narration_after_planning`'s `check_ceiling` → `RuntimeError`) — CLAUDE.md's policy is "downshift... complete lesson, flag in admin," not discard already-paid-for `lesson_planner`+`slide_generator` GPT-4o spend. HIGH. | Read the router: its ceiling check ran strictly after both premium nodes. Confirmed `tts_node`/`image_generator_node` have NO router-level gate — only a per-item check inside the node (Story 2-13/S2-13 AC-3). | Removed the router-level gate entirely; added a per-dispatch `check_ceiling` inside `narration_generator_node` itself, mirroring that exact established pattern — degrades just the one section (no cheaper tier exists below `llm_mini` to downshift to), never the whole lesson. New tests: `test_narration_fan_out_does_not_check_ceiling_at_all`, `test_cost_ceiling_skips_this_section_without_calling_the_llm`. |
| 2 | **`narration_stitch_node`'s cache-hit path returns `node_outputs["narration_stitch"]` with zero shape validation**, unlike the compute path (which filters non-dict entries before `_apply_narration_char_cap`) — `tts_node`'s `entry.get("segment_id", ...)` sits outside its own try/except, so a malformed cached row crashes the node instead of degrading. MEDIUM. | Read both code paths directly — confirmed the asymmetry exactly as described. | Added an `isinstance` shape check on the cache-hit read; treats a malformed cached value as a cache miss (recompute) rather than propagating it. |
| 3 | **A failed stitch pass has no admin-visible record**, unlike `narration_cap_applied` — only a `logger.warning`. MEDIUM. | Confirmed no persisted field existed for this. | Added `node_outputs["narration_stitch_applied"]: bool`, always present, same pattern as `narration_cap_applied`. |
| 4 | **Shared Redis progress-counter key double-counts** — Phase-1's 5-node fan-out and the new post-planner narration fan-out both write into the SAME `job:{lesson_id}:phase1_completed_keys` set, so a post-planner `SCARD` counts both phases' keys against a denominator that only reflects one. CONFIRMED by the reviewer via direct code read. | Verified: `set_key` was keyed only by `lesson_id`, no phase segregation; confirmed via grep that nothing outside `_increment_phase1_progress` reads this key (log-output-only, not a functional defect). | Added a `phase` keyword (default `"phase1"`, backward-compatible with all 5 existing call sites); narration's post-planner calls now pass `phase="narration_post_planner"`, giving it its own Redis set. |
| 5 | **`continuity_notes`' "force empty for segment 0" assumes `segment_summaries` arrives in true section order** — it's the same kind of unordered `Send()`-fan-in `operator.add` channel as `narration_scripts`, which `_apply_narration_char_cap` already has to re-sort. | Confirmed via `git show main:...` that the unsorted `enumerate(segment_summaries)` loop and its "authoritative input order" comment both **pre-date issue #236** — a real, previously-undiscovered gap in `lesson_planner_node`, not introduced by this PR, but this PR's `continuity_notes` feature is the first thing to actually depend on it being correct. | Registered as **D168** (`docs/DEFECT-REGISTER.md`). Fixed in this PR anyway (small, safe, reuses the already-proven `_segment_order_key` helper verbatim) since it's directly load-bearing for this PR's own new feature's correctness — not a rewrite of `lesson_planner_node`'s architecture. New test: `test_segment_summaries_out_of_order_fan_in_is_resorted_by_true_section_index`. |
| 7 | **Stitch prompt's "trim duplicate phrasing" instruction has no carve-out for intentional `continuity_notes`-driven reiteration** — could strip the exact callback phrasing the continuity feature exists to add. | Real design tension — `continuity_notes` deliberately asks for near-duplicate reiteration; the stitch prompt (a separate LLM call) had no awareness of that intent. | Added an explicit exception to the stitch system prompt: a deliberate callback/reiteration is not "duplicate phrasing" to trim. |
| 8 | **Stale docstrings**: `_warn_if_duplicated` said "the two nodes" (now three — `narration_stitch_node` added); two "6xN" comments should say "5xN" (Phase 1 shrank from 6 to 5 economy nodes); `admin/router.py`'s `narration_capped` docstring still claimed `tts_node` always writes it. | Grepped and read all four sites directly — confirmed all stale as described. | All four corrected. |

### Findings — confirmed real, correctly out of scope (not fixed)

- **#6, non-atomic checkpoint writes could theoretically erase `narration_cap_applied` under a concurrent duplicate pipeline attempt for the same `lesson_id`.** Verified this is a real theoretical race, but traced the exact interleaving required and confirmed it **widens, rather than introduces**, a race class `lesson_planner_node`/`slide_generator_node`'s own plain read-modify-write checkpoint pattern already carries (same disposition this story's own Round 1 Scale & Load Finding 4 already reached independently). Not fixed — would require an atomic-merge redesign of a pattern this codebase has already accepted elsewhere; out of this PR's scope.
- **#9, the segment-id degrade-not-fabricate guard is now duplicated a third time** (`lesson_planner_node`, `slide_generator_node`, `narration_stitch_node`). A valid refactor suggestion (extract to a shared helper), not a bug — accepted as technical debt, not fixed in a review-response pass.

### Findings — not a bug

- **#10, narration fan-out serialized behind `slide_generator`.** This was a **deliberate decision**, made explicitly with the product owner before implementation (see Context & Scope Boundary above: "sequential-after, smallest diff to existing wiring — decided with the user over the parallel-with-slide_generator alternative"). The reviewer wasn't aware of that prior decision, understandably, since it isn't visible from the diff alone.
