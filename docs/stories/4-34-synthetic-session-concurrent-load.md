...
4. **Tracker task still Partial**: "Analyse 20+ real student test session data"
   remains blocked on Dev 2's `?? null` WebSocket fix. This story unblocks it by
   creating a synthetic test suite that runs in CI without real credentials, proves
   data integrity for all 35 session patterns, and documents the concurrency safety
   of the CES computation path.
...
### Bug fix
- [ ] **AC 1.** `scripts/generate_synthetic_sessions.py` line 99: remove the
...
### Volume expansion
- [ ] **AC 2.** Generator tier counts updated to 5 low + 15 mid + 15 high = 35
  sessions. `SYNTHETIC_LESSON_IDS` list extended to 35 UUIDs. `build_session_rows()`
  returns exactly 35 dicts.

### Unit test suite
- [ ] **AC 3.** `apps/api/tests/test_s4_34_synthetic_session_analysis.py` exists
  with ≥ 35 test cases (one data-integrity scenario per synthetic session pattern plus
  edge cases).
- [ ] **AC 4.** Test verifies all 35 session dicts have the required DB column names:
  ...
  `event_type`, `payload`.
- [ ] **AC 5.** Test verifies all 35 `ces_final` values are in [0.0, 100.0] (not
  [0, 10 000] from the `* 100` bug).
- [ ] **AC 6.** Test verifies `score_source` on every teachback row is one of
  `('llm', 'fallback', 'skipped')` — the F2-2 CHECK constraint.
- [ ] **AC 7.** Test verifies `score` on every teachback row is an int in [0, 100].
- [ ] **AC 8.** Test verifies per-tier CES range: low-tier sessions produce
...
- [ ] **AC 9.** Test verifies teachback redistribution: when `teachback_score=None`,
...
- [ ] **AC 10.** Concurrent load test: `asyncio.gather` over 35 `compute_ces` calls
...
- [ ] **AC 11.** Test verifies idempotency logic shape: `insert_sessions()` uses
  `.limit(50)` on the dedup SELECT — guarded by an AST assertion (Scale Contract Q4).
- [ ] **AC 12.** All guard tests pass:
...
- [ ] **AC 13.** `docs/sprint4-ces-calibration-notes.md` §11 added: "Synthetic
...
### Tracker update
- [ ] **AC 14.** `docs/dev3-assessment-tracker.md`: partial task "Analyse 20+ real
...
- [ ] T1 — Story file created (this file), committed, pushed (story-first gate)
- [ ] T2 — Fix `* 100` bug in `scripts/generate_synthetic_sessions.py`; expand to 35 sessions
- [ ] T3 — Write `apps/api/tests/test_s4_34_synthetic_session_analysis.py` (≥ 35 tests)
- [ ] T4 — Verify ruff + mypy + pytest pass (all guard tests green)
- [ ] T5 — Update `docs/sprint4-ces-calibration-notes.md` §11
- [ ] T6 — Update tracker (partial → done)
