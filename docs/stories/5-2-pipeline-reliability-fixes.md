...
Status: review (all 9 checkable ACs addressed 2026-09-04 — see Completion Notes / Findings Summary; 8 branches open, none merged to main yet)
...
- **AC7, 2026-09-04**: audited — no code change, no new test needed; every failure mode this story found (AC2-AC6) already resolves to a legal `lesson_jobs.status` with a real error, never silently. Specifically checked the one path that plausibly risked a "logger.warning-only" degradation of lesson CONTENT/COST (not just breaker bookkeeping, which AC4 already covers separately): the cost-ceiling downshift path (S2-13). `_record_cost_downshift` merges its record into the caller's own `node_outputs` dict — the SAME dict the node's existing success-path write already persists to `lesson_jobs.node_outputs` — so a downshift is durable and queryable, never log-only; already covered by existing tests (`test_lesson_planner_node.py`, `test_slide_generator_node.py`, `test_tts_node.py`, each asserting `_cost_downshifts` survives to the real persisted checkpoint). Every other failure shape (retry exhaustion, cost-ceiling termination, Redis blips, node cancellation, breaker rejection — `CircuitOpenError` is a `RuntimeError` subclass, so it falls under the same generic-exception handling `test_failure_paths_write_schema_valid_status` already proves) was independently confirmed under AC2-AC6 above. No silent failures found.
- **AC9, 2026-09-04**: existing reaper tests (`test_reap_stale_lessons.py`) proved single-job staleness detection correctly (D91's `started_at`-based signal) and that one bad row doesn't stop a 2-row batch, but never exercised the `_REAP_BATCH_LIMIT=100` boundary itself — every existing test used 1-2 candidate rows. S4-1 run #10 measured 41/50 lessons stale at once under real load, making this a realistic, not hypothetical, scenario. Added `test_a_full_batch_of_concurrently_stale_jobs_is_all_reaped_in_one_pass`: simulates exactly 100 distinct stale rows in one query result, asserts every one is reaped in the single pass, none dropped or duplicated. Mutation-verified: truncated the reap loop to only the first 50 rows, confirmed the test reddens (50 != 100), reverted cleanly. No code fix needed — the existing loop already processes the full batch correctly; this closes the gap between "proven for 1-2 rows" and "proven at the real batch-size boundary."

## Findings Summary (AC-10)

Real failures found via S4-1's runs #9/#10, triaged per this story's protocol (registered before fixed):

| ID | What | Disposition | Guard |
|----|------|------|-------|
| D152 | Load-test harness deleted disposable users while their real ARQ jobs were still generating, corrupting in-flight paid work | Fixed | `test_loadtest_cleanup_safety.py` (4 tests) |
| D153 | 100% of large (~19.7MB) book uploads failed (dropped TLS connection); the storage upload had zero retry protection | Fixed | `test_upload_lesson_retries_a_transient_storage_write_error` |
| D129 (this register) | Real pipeline completion under 50-concurrent: P50=20.6min, P95=25.2min — over the 15-min target | Registered (Story 5-1's own headline finding, not duplicated here) | S4-1's own report |
| D154 | No per-node timeout budget exists between one provider call (120-180s) and the whole pipeline (1800s) — a diagnostic gap, not a correctness defect | Registered, not fixed (AC-5's own permitted disposition; a real per-node budget is a product decision) | N/A — confirming tests only (`test_cancelled_job_marks_lesson_failed_and_reraises`, `test_reap_stale_lessons.py`) |

This story's four mandatory categories (AC2-AC6), each confirmed ALREADY correctly implemented — no code fix needed, only previously-missing concurrency/scale proof, each mutation-verified:

| AC | Category | Verdict | Test |
|----|----------|---------|------|
| AC2 | Retry exhaustion under concurrency | Already correct | `test_concurrent_retry_exhaustion_across_two_lessons_does_not_cross_contaminate` |
| AC3 | Cost-ceiling atomicity under concurrency | Already correct | `test_accumulate_cost_never_loses_an_update_under_real_concurrency` + `test_concurrent_lessons_cost_accumulation_does_not_cross_contaminate` |
| AC4 | Redis blips under concurrent sharing | Already correct | `test_concurrent_calls_survive_an_intermittent_redis_blip_for_one_of_them` |
| AC5 | Node timeout budget shape | Real gap (D154), registered not fixed | `test_cancelled_job_marks_lesson_failed_and_reraises` + reaper tests (pre-existing) |
| AC6 | Circuit breaker cross-user blast radius | Confirmed, by design | `test_one_lessons_failures_trip_the_breaker_for_a_different_lesson_too` |
| AC9 | Reaper under a full concurrent-stale batch | Already correct | `test_a_full_batch_of_concurrently_stale_jobs_is_all_reaped_in_one_pass` |

**For S4-7 (on-call runbook) and W10-2 (monitoring dashboards)**: the two real, actionable gaps from this story are (1) D154 — no per-node timing visibility exists today, so diagnosing "which node is slow" during a real incident requires reading raw worker logs; a monitoring dashboard is the natural place to close this. (2) D129's own real capacity number (20-25 min per lesson at 50-concurrent, current worker capacity) — any runbook scenario involving "lessons taking too long" should cite this real, measured number rather than guess at what "too long" means.

**Status: all 9 checkable ACs (AC1-AC7, AC9) addressed as of 2026-09-04; AC10 (this summary) closes the loop.** AC8 (every fix ships with a mutation-verified regression test) was applied as a discipline throughout rather than as a separate pass — every confirming test above was deliberately mutation-checked. None of this story's 8 branches (D152/#185, D153/#186, AC2/#187, AC3/#189, AC4/#190, AC5/#191, AC6/#192, AC7+AC9+AC10/this branch) have merged to `main` yet — review/merge order determines whether the D149/D152 numbering collision with PR #188 (independently registered by another dev for unrelated work) needs manual renumbering.
