...
- [x] [Review][Defer] `.maybeSingle<NotificationPreferences>()`'s generic is a compile-time assertion only — a partially-null row or a renamed column would still type-check. [`apps/web/src/hooks/useNotificationPreferences.ts`] — deferred as DEFER-010 in `docs/deferred-work.md` (renumbered from DEFER-007 on merge into `sprint3-master`); identical to `useAttentionConsent.ts`'s own unaudited shape (DEFER-004) — now merged together, needs a single project-wide premise test rather than two separate ones.
