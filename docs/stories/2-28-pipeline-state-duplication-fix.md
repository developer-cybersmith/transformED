...
7. **AC-7 — e2e duplication assertions.** The happy-path e2e test asserts, per segment, `2 <= len(seg["quiz"]) <= 3`; globally, `len(qids) == len(set(qids))`; and in total `sum(len(s["quiz"]) for s in segments) == 3 * len(segments)`. Plus **two** money-path guards, both added because `package_builder` drops `segment_summaries`/`narration_scripts` from the package entirely — duplication there is invisible to any package-shape assertion yet still bills for it: (a) spy `_LessonPlanLLM`'s prompt and assert its `segment_id` list is duplicate-free and equal in length to `len(segments)`, proving the GPT-4o planner sees each segment once; (b) assert `_synthesize_with_fallback.await_count == len(segments)`, proving the TTS vendor is billed once per segment. **These assertions fail before AC-1 and pass after; that is the point.**
8. **AC-8 — Three canaries.** Pre-spend distinct-vs-total checks at `lesson_planner_node` (before the cache-hit read, so it fires before any GPT-4o token is spent) and at `tts_node` entry on `narration_scripts` (the only place duplicated narration is caught before paid synthesis). Plus a residual check in `package_builder_node` (after its cache-hit return) on **exact keys** — `(segment_id, question_id)` and `(segment_id, term)` — with **no** count bands, since jargon has no per-segment cap and every ERROR becomes a Sentry issue. All three must never raise. *(The residual canary was briefly deferred during the review round on "Sentry noise" grounds; the Acceptance Auditor correctly overruled that — AC-8 had already prescribed the exact-key design that solves the noise problem, and the deferral conflated AC-7's CI-time fixture assertions with a production-runtime detector. It is the ONLY runtime detector covering `quiz_questions`/`glossary` — the exact channels Dev 2 observed.)*
...
| 2026-07-28 | **Acceptance Auditor round (final layer, ran long).** It overruled the AC-8 deferral with a better argument than the one I gave: AC-8 had already prescribed the exact-key design that solves the "Sentry noise" objection I cited, and my second reason conflated AC-7's CI-time fixture assertions with a production-runtime detector. Since `lesson_planner`'s canary runs *before* the four doubling nodes and `tts_node`'s covers only `narration_scripts`, dropping the residual check left **no** runtime detector for `quiz_questions`/`glossary` — the exact channels Dev 2 observed. Implemented it (`_warn_if_exact_duplicates`, exact `(segment_id, id)` pairs, no count bands) rather than arguing. Also implemented AC-7's planner-prompt spy (the `await_count` guard covers `narration_scripts`, not the GPT-4o planner's input). Both new guards mutation-verified: re-adding `**state` to `image_generator_node` trips them. Fixed a stale File List (missing `test_fan_out_state_keys.py`) and a wrong Debug Log claim (the `E501` is at 2241 in both baseline and HEAD — no line shift). 659 passed, 1 skipped. | Dev 1 |
...
- Baseline lint comparison via `git show adb6d9d:...graph.py`: identical `E501` at **line 2241 in both** baseline and HEAD (an earlier note claimed a 2241 → 2239 shift; the Acceptance Auditor checked and no shift occurred) and identical format finding. Not introduced here.
...
- `apps/api/tests/unit/test_fan_out_state_keys.py` (NEW) — 8 tests guarding the Send() fan-out payload (AC-3); 6 fail on the `tier` mutation that previously left the suite green.
