...
**Decision-needed (2) — resolved by the user 2026-08-11, both now patches (see Patch list below):**

- [x] [Review][Decision] Cross-generation race on the delete-before-dispatch guard —
      ...
      doesn't know which "generation" of the key it's deleting. **RESOLVED: fix now, via
      compare-and-delete** (Lua script checking the value matches before deleting) — see Patch
      #10 below. [scale+edge]
- [x] [Review][Decision] When the INTERVENING deadline has expired AND the client's real event
      ...
      **RESOLVED: replay the original event through `dispatch_event` after the self-heal
      completes** — see Patch #11 below. [blind+edge]

**Patch (11) — unambiguous fixes:**

- [x] [Review][Patch] **CRITICAL — reopens the exact trap D63 exists to close.**
...
- [x] [Review][Patch] `intervention_timeout_seconds` has no bounds validation. A value ≥
...
- [x] [Review][Patch] `test_guard_scans_the_tutor_state_machine_directory_for_real` only proves
...
- [x] [Review][Patch] `test_advance_tutor_state_intervening_not_expired_dispatches_original_event`
...
- [x] [Review][Patch] `_intervention_deadline_expired`'s Redis-error fallback has zero logging on
...
- [x] [Review][Patch] Task 1.14 above and the D63 register entry both claim "full regression run
...
- [x] [Review][Patch] `wireTypes.ts`'s `FlowEvent` union addition has zero test coverage — the
...
- [x] [Review][Patch] `intervention_timeout_seconds` is only ever exercised via hand-built
...
- [x] [Review][Patch] D63's register row is textually marked CLOSED but sits under the
...
- [x] [Review][Patch] (resolved from Decision #1) Cross-generation race on the delete-before-dispatch
      guard — `redis.delete()` deletes `intervention_deadline_at` by key name only, with no
      value/version check. Fix: compare-and-delete via a Lua script (`EVAL` with a `GET`+compare+`DEL`
      atomic body) that only deletes when the stored value still matches what was just read as
      expired — applied to both call sites (`advance_tutor_state` and `process_attention_signal`).
      [`apps/api/app/modules/tutor/service.py`] [scale+edge, resolved by user 2026-08-11]
- [x] [Review][Patch] (resolved from Decision #2) When the INTERVENING deadline has expired and the
      client's real event isn't `intervention_complete`, that event is currently dropped. Fix: after
      the self-heal's `dispatch_event(session_id, "intervention_complete")` completes (transitioning
      to TEACHING), also dispatch the client's original `event` through `dispatch_event` — do not
      `return` unconditionally after the self-heal in either call site.
      [`apps/api/app/modules/tutor/service.py`] [blind+edge, resolved by user 2026-08-11]
