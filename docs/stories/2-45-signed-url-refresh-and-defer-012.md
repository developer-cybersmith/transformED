...
1. **Per-asset re-sign helper.** A new frontend helper (e.g. `lib/media/refreshSignedUrl.ts`) accepts an expired Supabase signed URL string, validates its origin matches the configured Supabase project (`NEXT_PUBLIC_SUPABASE_URL`) and extracts `{bucket, path}` from its `/storage/v1/object/sign/{bucket}/{path}` shape, and calls the real backend endpoint `GET /api/media/signed-url?bucket=...&path=...&expires_in=...` (via the existing `api` axios client at `lib/api.ts`, which already attaches the Supabase JWT) to obtain a fresh `signed_url`. `expires_in` is passed explicitly to match the system's deliberate 8-hour `_EMBEDDED_MEDIA_EXPIRY_S` window (`apps/api/app/modules/content/router.py:139`) — omitting it would silently fall back to the backend's 1-hour default, an 8x-shorter re-signed lifetime than the one this whole feature exists to restore. Malformed input (URL doesn't match the expected shape, or its host isn't the configured Supabase project) returns `null` rather than throwing.
2. **Audio auto-recovers on expiry.** `AudioTimeline.tsx`'s `handleError()` (currently `apps/web/src/components/player/AudioTimeline.tsx:356-361`, unconditionally sets `audioError`) first attempts exactly one automatic re-sign of the failed segment's `narration.audio_url` via AC1's helper. On success, the fresh URL overrides the segment's `audio_url` for as long as that segment stays current (**revised in review**: held in local component state keyed by `segment_id`, not written back into the store — no other consumer reads `lesson.segments[...].narration.audio_url` off the store today, and a store write would need the exact same segment-currency guard the local override already gets from `effectiveAudioUrl`'s comparison, so it would add a second guarded surface for no behavioral gain) and the `<audio>` element remounts with the fresh URL, **and the play/pause effect must re-run on this remount** (its dependency array includes the resign state) so playback actually resumes rather than sitting loaded-and-paused — playback resumes without the student clicking anything. On failure (helper returns `null`, or the re-signed fetch itself later errors), fall through to today's behavior: `setAudioError(true)` **only if the segment that failed is still the current segment** — a stale failure for a segment the student has already left (advanced past, manually retried, or a new lesson loaded) must never flip the error flag for whatever is current now.
...
4. **One automatic attempt, never a loop.** Both AC2 and AC3 attempt the automatic re-sign **at most once per asset per mount** (a per-element ref/flag, reset only on segment change or a genuinely new asset — not on the retried element's own subsequent `onError`). A second failure on the same asset goes straight to the existing manual-recovery path (Retry button / fallback image / placeholder) — it must never re-trigger the auto-resign and loop. "A genuinely new asset" **includes** a manual full-lesson retry (`Player.tsx`'s `handleRetryAudio`) delivering a freshly-signed URL for the same `segment_id` — the guard must clear for that segment when a manual retry happens, so if the new asset also later expires it gets its own automatic attempt rather than skipping straight to `setAudioError`. On the image side, the guard is scoped to one `SlideImage` component instance's current `imageUrl` prop — if that prop changes to a different URL on an already-mounted instance (e.g. a lesson content refresh), the guard resets for the new URL.
...
6. **DEFER-012 gets a real register ID.** Add entry **D66** (originally allocated as D63; renumbered in code review after colliding with a pre-existing closed D63 on `sprint3-master`, this story's actual merge target — the re-check had been performed against `main` instead) to `docs/DEFECT-REGISTER.md`'s "OPEN — accepted, with a named trigger" table, describing `apps/web/src/hooks/useAttentionMonitor.ts`'s `MODEL_ASSET_URL` pointing at a floating `float16/latest/face_landmarker.task` tag instead of a pinned version, with owner Dev 2 and an explicit trigger (a real, currently-valid versioned MediaPipe model URL is confirmed against official docs). Update the inline `DEFER-012` comment in `useAttentionMonitor.ts` and the entry in `docs/deferred-work.md` to cross-reference **D66** by ID, per CLAUDE.md binding rule 5.
...
- [x] Task 1 — Add `docs/DEFECT-REGISTER.md` entry (D66, after a D63 collision found and fixed in review) for DEFER-012, and cross-reference it from `useAttentionMonitor.ts`'s inline comment and `docs/deferred-work.md` (AC: #6)
  - [x] Confirm the highest allocated ID on the actual merge target (`sprint3-master`, not `main`) immediately before allocating (register's own stated rule — the first attempt re-checked the wrong branch and collided; re-checked correctly in review)
...
- [x] [Review][Patch] D63 collides with an already-allocated, already-closed D63 (and D64/D65) on `sprint3-master` — the actual merge target — since the re-check was performed against `main` instead [docs/DEFECT-REGISTER.md] — **Fixed:** renumbered to **D66**; cross-references updated in `useAttentionMonitor.ts`, `docs/deferred-work.md`, `docs/dev2-sprint-tracker.md`, and this story
- [x] [Review][Patch] Story-first gate violated: the story-only commit is not chronologically first — two unrelated `main`-origin docs commits were replayed ahead of it by the branch rebase [git history, sprint3/s3-09-signed-url-refresh] — **Fixed:** branch rebuilt from `sprint3-master` with only this story's 5 original commits cherry-picked, story-only commit genuinely first, then re-pushed with `--force-with-lease`
- [x] [Review][Patch] Race: the failed-resign `.then()` branch unconditionally calls `setAudioError(true)` with no check that the captured segment is still current — can flip a healthy, already-advanced-past (or already-reloaded-lesson) segment's error state [apps/web/src/components/player/AudioTimeline.tsx:394-397] — **Fixed:** the failure branch now re-reads current state and only sets `audioError` if the failed segment is still current; regression test added
- [x] [Review][Patch] A successful automatic re-sign remounts `<audio>` but the play/pause effect's dependency array doesn't include the resign, so the fresh element is never told to `.play()` — playback silently freezes [apps/web/src/components/player/AudioTimeline.tsx:100-140] — **Fixed:** `resignedAudio?.url` added to the effect's dependency array; regression test asserts `.play()` is called after a successful resign
- [x] [Review][Patch] `SlideImage`'s `attemptedResignRef` never resets when the `imageUrl` prop changes on an already-mounted instance, contradicting this story's own Dev Notes requirement [apps/web/src/components/player/SlideRenderer.tsx:25] — **Fixed:** `SlideImage` is now keyed by `imageUrl` at its call site, forcing a full remount (and fresh guard) on a genuinely new asset; regression test added
- [x] [Review][Patch] `refreshSignedUrl` omits `expires_in`, so the backend's 1-hour default silently replaces the system's deliberate 8-hour `_EMBEDDED_MEDIA_EXPIRY_S` window — combined with AC4's "one attempt ever," a second silent trap [apps/web/src/lib/media/refreshSignedUrl.ts:52-54] — **Fixed:** `RESIGN_EXPIRY_S = 8 * 60 * 60` now passed explicitly on every call; test asserts the query param
- [x] [Review][Patch] `parseSignedUrl` has no origin/host validation — any string sharing the path shape is accepted regardless of host, and the existing test locks in acceptance despite its name implying rejection [apps/web/src/lib/media/refreshSignedUrl.ts, refreshSignedUrl.test.ts] — **Fixed:** origin is now validated against `NEXT_PUBLIC_SUPABASE_URL`; the "wrong host" test now asserts `null`; also fixed the empty-`signed_url` edge case (`||` instead of `??`) as a byproduct
- [x] [Review][Patch] AC4 deviation: the attempt-guard is keyed only by `segment_id` and never resets after a manual full-lesson retry delivers a genuinely new asset for that same `segment_id` [apps/web/src/components/player/AudioTimeline.tsx] — **Fixed:** a new effect keyed on `audioRetryCount` clears the current segment's guard entry and any stale local override on every manual retry; regression test added
- [x] [Review][Patch] AC2's wording ("the store's copy of that segment's `audio_url` is updated in place") doesn't match the shipped design (local component state) — align the AC text to the implementation rather than add a redundant store-mutation surface that would need its own currency guard — **Fixed:** AC2 reworded to describe the actual local-override design and the reasoning for not mutating the store
- [x] [Review][Patch] Story Dev Notes citation errors: `retryAudio()` is at `player.machine.ts:349-353`, not `:310-313`; `handleRetryAudio()` spans `Player.tsx:77-91`, not `:73-87`; the `Player.tsx:21-24` reference is uninformative — **Fixed:** citations corrected in Dev Notes and References
- [x] [Review][Patch] Test coverage gaps: the image-side "one attempt only" guard is never proven with a signed-url-shaped fixture + call-count assertion (existing tests use a non-matching URL, so they can't distinguish "guard worked" from "URL never matched the shape"); empty-string `signed_url` and concurrent-double-error-while-in-flight are untested — **Fixed:** added a signed-url-shaped one-attempt-guard test with call-count assertion, an empty-`signed_url` test, and a race test (stale failed resign after segment advance)
...
- **Existing manual-recovery path, do not duplicate or break it:** `Player.tsx:77-91` (`handleRetryAudio`) already re-fetches the *whole* lesson (fresh signed URLs for every asset) via `onRefetchLesson` + `usePlayerStore.getState().refreshLessonMedia(fresh.content)`, then calls `retryAudio()` (`stores/player.machine.ts:349-353`, clears `audioError`, increments `audioRetryCount` to force the `<audio>` remount via its `key={segment_id}-{audioRetryCount}` at `AudioTimeline.tsx:409`). This story's automatic per-asset path sits in front of that flow, not instead of it — AC5 exists specifically to keep this boundary explicit for review.
...
- [Source: apps/web/src/components/player/Player.tsx:28 (onRefetchLesson prop), 77-91 (handleRetryAudio)]
- [Source: apps/web/src/stores/player.machine.ts:75-111, 349-353 (retryAudio)]
...
- Task 1: Added `docs/DEFECT-REGISTER.md` D63 (re-verified D62 was still the max on `main` immediately before allocating); cross-referenced from `useAttentionMonitor.ts`'s `DEFER-012` comment and a new `docs/deferred-work.md` entry. **Code review caught a collision** — the re-check target was wrong (`main`, not `sprint3-master`, this story's actual merge target, which already had D63–D65 allocated) — **renumbered to D66** everywhere, and the newly-exposed backend rate-limit gap on the signed-url endpoint (Scale & Load Hunter finding) registered separately as **D67**, deferred to Dev 1.
...
- **8-layer code review (Blind Hunter, Edge Case Hunter, Acceptance Auditor, Scale & Load Hunter, Story Quality, Test Coverage, AC Completeness, Process Integrity) found 11 patch findings + 1 defer — all 11 patches applied same day.** Highlights: rebuilt the branch (story-first gate had been violated by the rebase replaying 2 unrelated commits ahead of the story commit); fixed a real ID collision (D63→D66); fixed a race where a stale failed re-sign could flip `audioError` on an already-departed segment or lesson; fixed a bug where a successful re-sign never resumed `.play()`; fixed the image-side guard never resetting on a genuinely new `imageUrl`; added `expires_in` (was silently inheriting a 1-hour default instead of the system's 8-hour window) and origin validation to `refreshSignedUrl`; fixed AC4's guard not resetting after a manual retry; corrected two wrong Dev Notes citations; realigned AC2's wording to the shipped (local-state) design; closed the test-coverage gaps the reviewers found. D67 (backend rate-limit gap on the now-active signed-url endpoint) registered and deferred to Dev 1 — out of scope for this frontend-only story. Full `apps/web` suite: **927/927 passing**, `tsc --noEmit` clean, `eslint` clean (pre-existing `no-img-element` warning only).
...
- `docs/DEFECT-REGISTER.md` (modified — D66 added, renumbered from a colliding D63; D67 added for the deferred rate-limit gap)
- `docs/deferred-work.md` (modified — DEFER-012/D66 cross-reference; DEFER-016/D67 added)
...
- 2026-08-11: 8-layer code review completed — 11 patch findings + 1 defer. All 11 patches applied (branch rebuilt to fix the story-first gate, D63→D66 renumbering, race/freeze/guard-reset fixes, `expires_in`/origin validation, citation and AC-wording corrections, added test coverage). D67 registered and deferred. Status → done.
