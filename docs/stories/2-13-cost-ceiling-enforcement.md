...
- New tests: 1 downshift test each for `lesson_planner_node` and `slide_generator_node` (assert model swap + downshift record survives into the node's own final checkpoint write + node completes normally), 1 downshift test for `tts_node` (assert Sarvam/Azure classes never constructed, downshift recorded exactly once across 2 segments, not twice).
- Full regression suite (post-implementation, pre-review): 945 passed (up from a 942-test baseline established by Story 2-12, +3 for this story's new tests), same 48 pre-existing unrelated failures (5 files, all Dev 3/Dev 4 owned), 2 skipped — 0 regressions introduced.
- **Patch round (2026-07-17):** a 3-layer adversarial review (Blind Hunter, Edge Case Hunter, Acceptance Auditor) found and fixed 2 real HIGH-severity bugs the initial implementation shipped with — see Review Findings below. Post-patch full suite: 947 passed (+2 for two new fail-open regression tests), same 48 pre-existing unrelated failures, 2 skipped — 0 regressions.

### File List

- `apps/api/app/modules/content/pipeline/graph.py` (modified — `_record_cost_downshift()` changed from an async DB-writing function to a pure sync in-memory merge (patch round fix); `lesson_planner_node`/`slide_generator_node` wired with a fail-open `check_ceiling()` pre-check + downshift; `tts_node` wired with a per-segment `check_ceiling()` pre-check (already inside its per-segment try/except) + dedup'd downshift; `image_generator_node` unchanged)
- `apps/api/tests/unit/test_lesson_planner_node.py` (modified — autouse `check_ceiling=False` fixture, 1 downshift test asserting the record survives the node's own final write, 1 fail-open test)
- `apps/api/tests/unit/test_slide_generator_node.py` (modified — autouse `check_ceiling=False` fixture, 1 downshift test asserting the record survives the node's own final write, 1 fail-open test)
- `apps/api/tests/unit/test_tts_node.py` (modified — autouse `check_ceiling=False` fixture, 1 downshift test asserting the record survives the node's own final write + `check_ceiling` called with the correct `lesson_id` each of 2 segments)
...
| 2026-07-17 | Code review patch round (Blind Hunter + Edge Case Hunter + Acceptance Auditor): fixed a confirmed bug where `_record_cost_downshift`'s own DB write was silently clobbered by each node's own subsequent final checkpoint write (defeating AC-5 on the very request meant to demonstrate it) — converted to a pure sync in-memory merge; added a fail-open guard around the previously-unguarded `check_ceiling()` calls in `lesson_planner_node`/`slide_generator_node` (a Redis error would otherwise crash the node, a new failure mode inconsistent with every other `check_ceiling()` call site in the file). Added 2 fail-open regression tests + strengthened the 3 downshift tests to assert against the real final checkpoint write instead of a mocked helper. 947 passed / same 48 pre-existing unrelated failures / 2 skipped — 0 regressions. Status → done. |

### Review Findings (2026-07-17 — 3-layer adversarial review: Blind Hunter, Edge Case Hunter, Acceptance Auditor)

- [x] [Review][Patch] **FIXED — HIGH (confirmed independently by Blind Hunter and Edge Case Hunter, with Edge Case Hunter pinpointing the exact mechanism and line numbers) — `_record_cost_downshift`'s own read-modify-write to `lesson_jobs.node_outputs` was silently clobbered by each node's own subsequent final checkpoint write**, which reused the SAME `node_outputs` local variable read near the top of the function — before the downshift happened. Concretely: `lesson_planner_node`'s final write at (then) line 1089 was `{**node_outputs, "lesson_planner": lesson_plan}` using the stale pre-downshift dict, so the `_cost_downshifts` key `_record_cost_downshift` had just written to the DB was immediately overwritten back out on the SAME successful request — AC-5 was defeated in the common (success) path, not just under exotic concurrency. Same bug in `slide_generator_node` and `tts_node`. Fixed by converting `_record_cost_downshift` from an async function that performs its own DB round trip into a pure, synchronous, no-I/O function that merges the downshift entry into the CALLER's local `node_outputs` dict and returns it — the node's own already-planned final write now persists it correctly, with no separate write and no race at all. [`app/modules/content/pipeline/graph.py`] (Blind Hunter + Edge Case Hunter)
- [x] [Review][Patch] **FIXED — HIGH (Blind Hunter + Edge Case Hunter, independently) — `check_ceiling(lesson_id)` in `lesson_planner_node`/`slide_generator_node` had no fail-open guard**, unlike every other `check_ceiling()` call site in the file (`_fan_out_phase1_economy_nodes` explicitly fails open with a logged warning; `tts_node`'s and `image_generator_node`'s calls sit inside a per-item try/except that already degrades gracefully). A transient Redis error (the exact error this story's own test debug log hit while developing) would crash these two nodes outright — a new hard-failure mode this story introduced while trying to make the pipeline MORE resilient to cost issues. Fixed by wrapping both calls in `try/except`, logging a warning and defaulting to "not over ceiling" on failure, mirroring `_fan_out_phase1_economy_nodes`' established pattern exactly. [`app/modules/content/pipeline/graph.py`] (Blind Hunter + Edge Case Hunter)
- [x] [Review][Patch] **FIXED — LOW — no test asserted `check_ceiling()` was actually called with the correct `lesson_id`** — an `AsyncMock(return_value=True)` accepts and ignores any arguments, so a bug that called it with a wrong/hardcoded value would have passed all three downshift tests silently. Added an explicit call-argument assertion to all three. [`tests/unit/test_lesson_planner_node.py`, `tests/unit/test_slide_generator_node.py`, `tests/unit/test_tts_node.py`] (Blind Hunter)
- [x] [Review][Patch] **FIXED — LOW — minor formatting: the new `autouse` pytest fixture in all three test files was missing the second blank line PEP 8/black expects before a top-level `def`** (three instances, same pattern — plausible find-and-replace authorship). Added the missing blank line in each file. (Blind Hunter)
- [x] [Review][Dismiss] **Edge Case Hunter's `tts_node` retry-duplication concern (the `downshift_recorded` local flag resets to `False` on an ARQ retry, allowing duplicate `_cost_downshifts` entries across retries)** — moot after the HIGH-severity fix above: `_record_cost_downshift` no longer performs its own DB write, so nothing about a downshift persists until the node's own single final checkpoint write. On a crashed-then-retried invocation, the ENTIRE prior attempt's in-memory state (including any downshift merges) is discarded along with it — there is no longer any mechanism by which a duplicate entry could accumulate across retries. Actually improved, not just dismissed, by the other fix. (Edge Case Hunter)
- [x] [Review][Dismiss] **Blind Hunter's "no-op downshift if `settings.llm_mini` happens to equal the premium model" observation** — real but low-value: a misconfigured environment where `LLM_LESSON_PLANNER`/`LLM_SLIDE_GENERATOR` and `LLM_MINI` point to the identical model string would log a "downshifting X -> X" no-op and write a no-op `_cost_downshifts` entry. Harmless (no behavior change, no crash) and would require config validation elsewhere (`config.py`) to prevent, which is out of this story's scope — a misleading admin-trail entry in an already-misconfigured environment is not worth the added complexity of a same-value guard here. Dismissed as low-value, not a defect this story should absorb. (Blind Hunter)
- [x] [Review][Dismiss] **Blind Hunter's "duplicated inline `from app.core.cost_tracker import check_ceiling` imports instead of one top-level import" observation** — confirmed by Acceptance Auditor as intentional and consistent: every node in this file already does its own lazy in-function imports (the established convention this file documents repeatedly, e.g. for `get_supabase`/`get_llm_provider`/provider classes), specifically because it's what keeps `patch("app.providers.llm.openai.OpenAILLMProvider", ...)`-style test mocking working. Not a defect — matches the file's own stated pattern. Dismissed. (Blind Hunter, overruled by cross-referencing Acceptance Auditor's independent confirmation)
- [x] [Review][Dismiss] **Blind Hunter's "positional `call_args.args[1]` assertions are brittle" observation** — matches the pre-existing convention already used by every other test in the same two files (e.g. the pre-existing `test_model_used_is_settings_llm_lesson_planner`/`test_model_used_is_settings_llm_slide_generator` tests use the identical `call_args.args[1]` pattern). Not a new problem introduced by this diff; changing only the new tests to a different style would be the actual inconsistency. Dismissed. (Blind Hunter)
- [x] [Review][Dismiss] **Blind Hunter's "`cost` variable unused after assignment in the downshifted tts branch" observation** — a false positive stemming from Blind Hunter having zero project context (by design, diff-only). `cost=0.0` in the downshift branch IS consumed identically to every other branch: `accumulate_cost(lesson_id, cost)` is only called when `audio_bytes is not None` (verified in the surrounding, unmodified code), and the downshift branch always sets `audio_bytes = None` — so `accumulate_cost` is correctly skipped, exactly matching the pre-existing "both providers fail" test's assertion (`mock_accumulate.assert_not_called()`). Dismissed — verified correct by reading the surrounding code Blind Hunter didn't have access to. (Blind Hunter)
- [x] [Review][Dismiss] **Blind Hunter's "TOCTOU between `check_ceiling` and the actual LLM/TTS call" observation** — correctly identified as a soft-ceiling design, not a bug: this is the SAME check-then-act shape every pre-existing `check_ceiling()` call site in the codebase already uses (Story 2-1's Phase 1 gate, Story 2-9's `image_generator_node`), and `check_ceiling`'s own docstring/design has always been advisory rather than atomically enforced against concurrent spend. Not a regression or defect introduced by this story. Dismissed. (Blind Hunter)
- [x] [Review][Confirm — no findings from this reviewer] **Acceptance Auditor found zero AC violations** — confirmed all 7 ACs against the actual (pre-patch-round) code state, including the exact insertion points, `check_ceiling` semantics, config field names, and the `image_generator_node` verify-only requirement (AC-4). One minor style observation (the same duplicated-import pattern Blind Hunter raised) was noted as non-blocking, consistent with the Dismiss above.
