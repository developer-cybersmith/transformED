...
- [ ] [Review][Defer] F12 — LOW: `_validated_book_id(book_id)` called twice in both endpoints — assign result to a variable and reuse [`apps/api/app/modules/content/router.py:upsert_book_context, get_book_context`] — **D159**: Deferred; redundancy is harmless, fix after D155 router integration tests exist.
