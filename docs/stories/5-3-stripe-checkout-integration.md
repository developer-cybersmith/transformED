...
12. **[Added 2026-08-26, code review — Story Quality finding]** A verified `checkout.session.completed` event carrying no `metadata.user_id`, or one whose `payment_status` is not yet `"paid"` (Stripe's documented behavior for delayed/async payment methods), grants no credit and is logged (ERROR for the missing-user_id case, INFO for the not-yet-paid case) rather than silently dropped or 500'd — the event is still recorded in the idempotency ledger so a redelivery of the exact same malformed/unpaid event doesn't reprocess indefinitely. Handling the follow-up `checkout.session.async_payment_succeeded` event that Stripe sends once an async payment later settles is explicitly out of scope (registered in `docs/stories/deferred-work.md`).
...
3. **What is the SCOPE of every limit — per user, per instance, or per deployment?**
   ...
   - The webhook endpoint itself carries **no** per-user rate limit at all (it has no `CurrentUser` — Stripe is the caller), so a burst of legitimate Stripe retries from Stripe's own IP ranges is never throttled. **[Review Finding — corrected 2026-08-26]** Originally phrased as "must be explicitly exempted from any IP-based app-wide limiter" — verified against `main.py`: no such global limiter exists at all (slowapi's shared `Limiter` has no `default_limits`, and enforces a ceiling only on routes explicitly decorated with `@limiter.limit(...)`). The webhook route carries no such decorator, so the requirement is satisfied by omission, not by an exemption — corrected to avoid describing `main.py` state that isn't accurate.
...
- [x] [Review][Patch] **`import stripe` in `payments/router.py` violates the provider-abstraction rule (AC10) and the provider file's own docstring claim** ("The ONLY file... allowed to `import stripe`"). Used to catch `stripe.SignatureVerificationError` directly. No source-scan test enforces this (unlike the sibling rate-limit-keying check in the same file). **Confirmed by direct `grep` after one reviewer's contradictory claim that it didn't exist.** [`apps/api/app/modules/payments/router.py:14,99`] (source: ac-completeness+process-integrity+test-coverage — 3 independent layers, plus direct verification)
- [x] [Review][Patch] **Unauthenticated `/webhook` route reads the entire raw body into memory before any signature check, with no size cap and no rate limit** (by design — Stripe retries must never be throttled). Any caller, not just Stripe, can POST arbitrarily large bodies at a public route on the single shared API process. `observed_behaviour = unbounded-growth` — a real Scale Contract Q4 finding. [`apps/api/app/modules/payments/router.py:94`] (source: edge-case+scale+blind — 3 independent layers)
- [x] [Review][Patch] **No error handling around the Stripe API call in `create_checkout_session`** — a bad price, network error, or auth failure surfaces as an opaque generic 500 instead of a clear provider-error response.
- [x] [Review][Patch] **No error handling for a malformed-but-validly-signed webhook payload** (missing `data`/`object`/`id`/`type` keys) — `ValueError`/`KeyError` would propagate as an uncaught 500 instead of a 400.

**[Review][Patch] — test/doc/process correctness:**

- [x] [Review][Patch] AC2 (`success_url`/`cancel_url`) has **zero test coverage** — nothing would fail if the redirect URLs were swapped, typo'd, or lost the `{CHECKOUT_SESSION_ID}` token. Fix alongside the absolute-URL patch above. (source: test-coverage+ac-completeness+process-integrity — 3 independent layers)
- [x] [Review][Patch] AC1's "never a client-supplied price" test is **tautological** — it extracts the price from the same call it's checking and asserts it equals itself, proving list-shape only, never that the price actually equals `settings.stripe_price_id_lesson_credit`. (source: ac-completeness+process-integrity)
- [x] [Review][Patch] AC10's "required `Settings` fields" claim (`stripe_secret_key`/`stripe_webhook_secret`) is **never tested** — `test_config_settings.py`'s `_REQUIRED` dict doesn't include the 3 new Stripe fields.
- [x] [Review][Patch] `test_migration_payments_schema.py` (this story's `# MOCK-CONTRACT:`-named real-dependency coverage) lives directly under `apps/api/tests/`, **not** `tests/unit/` or `tests/integration/` — confirmed by running CI's actual gating command (`pytest tests/unit tests/integration -q -m "not postgres"`), which never touches it. The MOCK-CONTRACT marker's claimed backing cannot fail the merge gate. Move this new file into `tests/unit/` (adjusting its `_REPO_ROOT` parents-index) — the sibling pre-existing files with the same issue are not this story's job to move.
- [x] [Review][Patch] **Fabricated/unconfirmed defect-register citation: "D134"** — confirmed by direct search, no such row exists in `docs/DEFECT-REGISTER.md` (highest real ID is D125). Same shape as Story 5-4's "D129" finding — likely a real commit-message reference on an unmerged branch, not yet centrally registered. Soften the citation in this story + `docs/dev1-tracker.md` rather than assert a specific ID that isn't confirmed to exist in the current register.
- [x] [Review][Patch] **Dev Agent Record's mypy claim is scoped wrong** — "one reported error" was checked file-by-file (touched files only), not repo-wide. Running `mypy app` (CI's actual scope, per binding rule 1) finds **3** pre-existing, unrelated errors (`providers/llm/openai.py:69`, `providers/image/openai_image.py:134`, `providers/embeddings/openai.py:61` — all the same httpx/httpx2 stub mismatch). Correct the claim.
- [x] [Review][Patch] No test sends a webhook request with the `Stripe-Signature` header **entirely absent** (vs. present-but-wrong) — AC3 says "missing **or** invalid," only "invalid" is exercised.
- [x] [Review][Patch] The tested-and-real "missing `metadata.user_id`" webhook behavior has **no corresponding AC** — AC6 only covers wrong event *types*, not a malformed event of the right type. Add an AC (or fold into AC6) documenting it.
- [x] [Review][Patch] Scale & Load Q3 overstates reality: it says the webhook "must be explicitly exempted from any IP-based app-wide limiter," but no such global limiter exists in `main.py` at all (slowapi only enforces `@limiter.limit`-decorated routes) — the requirement is satisfied by omission, not an exemption. Reword.
- [x] [Review][Patch] Register a real `D-nn` entry in `docs/DEFECT-REGISTER.md` for "payments RPC atomicity (grant/decrement/idempotency) never executed against a real Postgres instance" — a genuine, stated `KNOWN LIMITATION` with no register ID today (binding rule 5).
- [x] [Review][Patch] The claim that "a plain PostgREST `.upsert()` call... does not reliably expose whether this call's insert affected a row" (justifying the third RPC) is stated as settled fact with no citation or premise test, unlike this story's other empirically-verified claims (e.g. `StripeObject.to_dict()`). Soften to "believed to be unreliable, not empirically verified in this session" — the RPC choice itself is still the right call, just the stated justification overclaims certainty.
...
- Full unit suite before and after: 1247 passed / 6 skipped / 3 pre-existing unrelated failures
  (`test_extract_page_bounds.py`, a pypdfium2 API mismatch — **[Review Finding — corrected
  2026-08-26]** previously cited as "D134"; no such row exists in `docs/DEFECT-REGISTER.md`
  (highest real ID before this story's own D136/D137 was D125) — likely a real commit-message
  reference on a branch not yet merged to `main`, same shape as Story 5-4's "D129" finding.
  Citation removed rather than asserting an ID unconfirmed in the current register) — identical
  failure set, zero regressions.
- **[Review Finding — corrected 2026-08-26]** The Dev Agent Record originally claimed "mypy's one
  reported error" — that was checked file-by-file (touched files only), not repo-wide. Running
  `mypy app` (CI's actual scope, per binding rule 1) finds **3** pre-existing, unrelated errors:
  `providers/llm/openai.py:69`, `providers/image/openai_image.py:134`,
  `providers/embeddings/openai.py:61` — all the identical httpx/httpx2 stub-mismatch root cause,
  none touched by this diff.
- **Post-review verification (2026-08-26):** ran CI's EXACT gating command this time
  (`pytest tests/unit tests/integration -q -m "not postgres"`, not a hand-picked file list) —
  1292 passed / 6 skipped / same 3 pre-existing unrelated failures, zero regressions. Ruff clean;
  `mypy app` (repo-wide) confirmed at exactly the 3 pre-existing errors named above, no new ones
  from the 19 patches applied.
...
**Post-review patches (2026-08-26, 19 findings applied):**
- `apps/api/app/config.py` — added `app_base_url` (reuses `.env.example`'s `NEXT_PUBLIC_APP_URL`) for absolute Stripe redirect URLs
- `apps/api/app/providers/payments/base.py` — added `WebhookVerificationError`/`PaymentProviderError`, provider-agnostic exceptions so no SDK exception type crosses the boundary
- `apps/api/app/providers/payments/stripe.py` — translates `stripe.SignatureVerificationError`/`stripe.StripeError`/malformed-structure errors into the two new exception types
- `apps/api/app/modules/payments/router.py` — removed `import stripe` entirely; absolute success/cancel URLs; `Content-Length` + post-read size guard (413) on the webhook route; `PaymentProviderError` → 502 on checkout-session creation
- `apps/api/app/modules/payments/service.py` — rewritten `process_webhook_event`: `payment_status == "paid"` gate before any grant; the two-separate-RPC-calls bug (D136) closed by routing the real grant path through one new atomic RPC
- `apps/api/app/modules/content/router.py` — refund-failure log bumped `WARNING` → `ERROR`
- `supabase/migrations/20260825000000_stripe_payments_lesson_access.sql` — new RPC `record_stripe_event_and_grant_credits` (the atomic fix for D136); reworded the PostgREST-upsert justification comment to not overstate certainty
- `apps/api/tests/unit/test_payments_router.py` — AC1 tautology fixed, AC2 test added, 8 new tests (provider-error 502, SDK-exception-never-leaks, no-bare-stripe-import source-scan, no-bare-STRIPE-env-read source-scan, missing-signature-header, malformed-payload, body-size-cap, not-yet-paid)
- `apps/api/tests/unit/test_generate_lesson_endpoint.py` — new `test_refund_rpc_itself_failing_still_returns_500_and_attempts_the_refund` + `_RaisingRpcCall`/`refund_rpc_fails` fake support
- `apps/api/tests/unit/test_migration_payments_schema.py` — **moved here from `apps/api/tests/`** (was outside CI's actual gating scope — confirmed by running the real gating command), `_REPO_ROOT` parents-index corrected, new RPC's grant/revoke + one-atomic-body tests added
- `apps/api/tests/test_config_settings.py` — Stripe fields added to `_REQUIRED`, new required-field-raises-without tests, default-credits test
- `docs/DEFECT-REGISTER.md` — D136 (closed, the atomicity fix) and D137 (accepted residual risk, deferred by explicit user decision) registered
- `docs/stories/deferred-work.md` — 2 new entries (`async_payment_succeeded` handling; migration-test-not-CI-gated sibling files)
- `docs/dev1-tracker.md` — corrected the fabricated "D134" citation, noted the review outcome
