...
- [ ] Segment-end detection → CHECKING IN state — 🔲 NOT STARTED, not blocked. **Corrected 2026-07-06** (previous 2026-07-02 note overstated readiness — "Dev 4's FSM state is live in the player store" was not actually true): `sendControl({type:'segment_complete'})` and `useLessonSocket` (S1-07) are built and unit-tested in isolation, but `useLessonSocket` is **not mounted anywhere in the live player** (`Player.tsx`/`PlayerLoader.tsx`) — the lesson WebSocket never actually connects during a real session today, so `setTutorState()` is never called in production regardless of what the server sends. `tutorState` in `player.machine.ts` also has zero readers anywhere in the UI — no CHECKING_IN screen exists. Quiz triggering today is entirely client-local (`AudioTimeline.tsx` calls `store.enterQuiz()` directly on segment boundary), with no round-trip to the backend tutor FSM at all. Real scope: (1) mount the socket live in the player, (2) call `sendControl` on segment end, (3) design+build a CHECKING_IN UI reaction, (4) decide whether quiz-triggering stays client-authoritative (current) or becomes server-authoritative via `state_change` (matches CLAUDE.md's FSM design but adds latency + needs a WS-down fallback). Not blocked on Dev 4 — backend FSM logic for this transition is code-merged and unit-tested (mocked Redis) per `docs/dev4-websocket-tutor-tracker.md`, pending only live-Redis integration testing, same posture S2-04 already worked successfully against. See Dev 2 tracker §11 S2-06 for the full writeup.
